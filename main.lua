local function __KubzzHub_Init_Main__()
...
if not ModernV2 then
    ...
    if loaderOk then
        ...
        warn("[KubzzHub] Vercel mirror failed, trying GitHub fallback:", loaderResult)
        ...
        if fallbackOk then
            ...
            warn("[KubzzHub] Failed to load ModernV2 from all sources:", fallbackResult)
        end
    end
end
...
local UIConfigPath = "KubzzHub_UI_Theme.json"
local KubzzUI_Solid = false
local KubzzUI_Color = "Blue"

pcall(function()
    if isfile and readfile and isfile(UIConfigPath) then
        local data = HttpService:JSONDecode(readfile(UIConfigPath))
        KubzzUI_Solid = data.Solid
        KubzzUI_Color = data.Color or "Blue"
    end
end)

if KubzzUI_Color == "Default" then
    KubzzUI_Color = "Blue"
end
...
if ModernV2 and ModernV2.CreateMenuIcon then
    MenuIcon = ModernV2:CreateMenuIcon({
        Image = "rbxassetid://107597314467545",
        Size = 48,
...
if ModernV2 then
    ...
    Window = ModernV2:Window({
        ...
        Config = {
            ConfigFolder = "KubzzHubViolenceDistrict",
            AutoSaveFile = "KUBZZ_VD",
            AutoSave = false,
            AutoLoad = false,
            Overwrite = true,
            Format = "JSON",
    ...
    Window:SetAccount({
        ...
        Expires = "KubzzHubOnTop",
    })

    Window:CreateHomeTab({
        Name = "Dashboard",
        Icon = "lucide:layout-dashboard",
        Content = "KubzzHub Violence District Script",
        DiscordInvite = "",
        ...
        Changelog = {
            {
                Title = "KubzzHub VD v1.5.7",
                Description = "Added Fake Perks (Flowstate, Quick Recovery, Perfect Landing, Adrenaline Rush) with stack support and shared cooldown. Added Copy Avatar feature with accurate skin/mesh restoration.",
            },
            {
                Title = "KubzzHub VD v1.5.6",
                Description = "Improved Weather & Sky Theme particles and fixed Fullbright compatibility with active weather themes.",
            },
            {
                Title = "KubzzHub VD v1.5.5",
                Description = "Improved Aimbot, Spear Aimbot, Silent Aim Spear, and added new Auto Dodge Spear feature.",
            },
            {
                Title = "KubzzHub VD v1.5.4",
...
function VD_UpdateCrosshair()
    ...
    CrosshairGui.Name = "KUBZZ_Crosshair"
...
function GetSafeChamsFolder()
    ...
    local f = pg:FindFirstChild("KUBZZ_WorkspaceChams")
    if not f then
        f = Instance.new("Folder")
        f.Name = "KUBZZ_WorkspaceChams"
        f.Parent = pg
    end
    VD_ChamsFolder = f
    return f
...
local ConfigFolderName = "KubzzHub_VD"
local HttpService = game:GetService("HttpService")
...
function KUBZZ_SaveConfig(name)
...
local VD_To_Flag = {
    ...
    VIS_KubzztKiller = "Kubzzt Killer Display",
...
function KUBZZ_LoadConfig(name)
    name = (name and name ~= "") and name or getgenv().CurrentConfigName
    if not name or name == "" then name = "Default" end
    local path = ConfigFolderName .. "/" .. name .. ".json"
    pcall(function()
        if readfile and isfile and isfile(path) then
            ...
            if getgenv().KUBZZ_SyncLoadedFeatures then pcall(getgenv().KUBZZ_SyncLoadedFeatures) end
        end
    end)
end

function KUBZZ_DeleteConfig(name)
    name = (name and name ~= "") and name or getgenv().CurrentConfigName
...
function VD_ApplyWeather(themeName)
    local theme = KUBZZ_WeatherPresets[themeName]
    if not theme then theme = KUBZZ_WeatherPresets["Default"] end
...
-- CHARACTER REFS
-- =====================================================
-- Character, Humanoid, and Root are declared at the top of __KubzzHub_Init_Main__
...
-- =====================================================
local KUBZZ_WorldReg
getgenv().KUBZZ_oldNamecall = nil

function setupAntiFail()
    if getgenv().KUBZZ_AntiFailHooked then return end
    getgenv().KUBZZ_AntiFailHooked = true
    task.spawn(function()
...
-- → infinite recursion → stack overflow → force close.
            ...
            _genv.KUBZZ_oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
                ...
                if VD.KILLER_SilentAimFlask and method == "FireServer" then
                    ...
                    if ok and name == "ThrowFlask" then
                        ...
                        if closest then
                            ...
                            return _genv.KUBZZ_oldNamecall(self, unpack(args))
                        end
                    end
                end

                if method == "FireServer" and not checkcaller() then
                    local flashRemote = _genv.KUBZZ_FlashlightActivateRemote
                    if flashRemote and self == flashRemote and _genv.KUBZZ_SetFlashlightAimActive then
                        local args = { ... }
                        pcall(_genv.KUBZZ_SetFlashlightAimActive, args[2] == true, args[1])
                    end
                end

                if _genv.KUBZZ_oldNamecall then
                    return _genv.KUBZZ_oldNamecall(self, ...)
                end
            end)
...
-- =====================================================
getgenv().KUBZZ_fpWasSet = false
getgenv().KUBZZ_fpOriginal = nil

function RestoreFirstPersonCamera()
    if not getgenv().KUBZZ_fpWasSet then return end
    getgenv().KUBZZ_fpWasSet = false

    pcall(function()
        if getgenv().KUBZZ_fpOriginal then
            LocalPlayer.CameraMode = getgenv().KUBZZ_fpOriginal.CameraMode or Enum.CameraMode.Classic
            LocalPlayer.CameraMaxZoomDistance = getgenv().KUBZZ_fpOriginal.CameraMaxZoomDistance or 128
            LocalPlayer.CameraMinZoomDistance = getgenv().KUBZZ_fpOriginal.CameraMinZoomDistance or 0.5
        else
...
RunService.RenderStepped:Connect(function()
    pcall(function()
        if VD.SURV_FirstPerson then
            ...
            if isSurvivor then
                if not getgenv().KUBZZ_fpWasSet then
                    getgenv().KUBZZ_fpOriginal = {
                ...
                getgenv().KUBZZ_fpWasSet = true
            elseif getgenv().KUBZZ_fpWasSet then
...
-- Kontrol ditambahkan ke tab Visual lewat getgenv().KUBZZ_AddVisualESPControls.
-- =====================================================
do
    if getgenv().KUBZZ_VD_VisualESP_Cleanup then
        pcall(getgenv().KUBZZ_VD_VisualESP_Cleanup)
    end

    local LP = LocalPlayer
    local KUBZZ_Dead = false
    local KUBZZ_ControlsAdded = false

    local KUBZZ_ESPState = {
    ...
    getgenv().KUBZZ_VD_VisualESP_State = KUBZZ_ESPState
    ...
    local KUBZZ_MapAdd, KUBZZ_MapRem = {}, {}
    local KUBZZ_PlayerConns = {}
    local KUBZZ_Connections = {}
    local KUBZZ_PalletState = setmetatable({}, { __mode = "k" })
    local KUBZZ_WindowState = setmetatable({}, { __mode = "k" })
    local KUBZZ_InstanceIds = setmetatable({}, { __mode = "k" })
    local KUBZZ_KubzztId = 0
    local KUBZZ_PlayerLoopThread = nil
    local KUBZZ_WorldLoopThread = nil
    local KUBZZ_ESPFolder = nil

    local KUBZZ_DisplayNames = {
        ["Motion Tracker"] = true,
        ["Gate"] = true,
        ["Flashlight"] = true,
        ["Bandage"] = true,
    ...
    local function KUBZZ_Alive(inst)
        if not inst then return false end
        local ok, parent = pcall(function() return inst.Parent end)
        return ok and parent ~= nil
    end

    local function KUBZZ_Clamp(n, lo, hi)
        n = tonumber(n) or lo
    ...
    local function KUBZZ_PlayerKey(player)
    ...
    local function KUBZZ_EspId(inst)
        if not inst then return "nil" end
        local id = KUBZZ_InstanceIds[inst]
        if id then return id end
        KUBZZ_KubzztId = KUBZZ_KubzztId + 1
        id = tostring(KUBZZ_KubzztId)
        KUBZZ_InstanceIds[inst] = id
        return id
    end

    local function KUBZZ_GetESPParent()
    ...
    local function KUBZZ_GetESPFolder()
        if KUBZZ_ESPFolder and KUBZZ_ESPFolder.Parent then
            return KUBZZ_ESPFolder
        end

        local parent = KUBZZ_GetESPParent()
        local old = parent:FindFirstChild("KubzzHub_VisualESP") or parent:FindFirstChild("ZiaanHub_ESP")
        if old then old:Destroy() end

        local folder = Instance.new("Folder")
        folder.Name = "KubzzHub_VisualESP"
        folder.Parent = parent
        KUBZZ_ESPFolder = folder
        return folder
    end

    local function KUBZZ_ClearPrefix(prefix, keepName)
        local folder = KUBZZ_GetESPFolder()
    ...
    local function KUBZZ_SafeNotify(title, content, duration)
        pcall(function()
    ...
    local function KUBZZ_ValidPart(part)
        return part and KUBZZ_Alive(part) and part:IsA("BasePart")
    end

    local function KUBZZ_FirstBasePart(inst)
        if not KUBZZ_Alive(inst) then return nil end
        if inst:IsA("BasePart") then return inst end
        if inst:IsA("Model") then
            if inst.PrimaryPart and inst.PrimaryPart:IsA("BasePart") and KUBZZ_Alive(inst.PrimaryPart) then
                return inst.PrimaryPart
            end
            local part = inst:FindFirstChildWhichIsA("BasePart", true)
            if KUBZZ_ValidPart(part) then return part end
        end
        if inst:IsA("Tool") then
            local handle = inst:FindFirstChild("Handle") or inst:FindFirstChildWhichIsA("BasePart")
            if KUBZZ_ValidPart(handle) then return handle end
        end
        return nil
    end

    local function KUBZZ_GetRole(player)
    ...
    local function KUBZZ_PlayerRoleEnabled(player)
        local role = KUBZZ_GetRole(player)
        if role == "Killer" then return KUBZZ_ESPState.KillerESP end
        if role == "Spectator" then return KUBZZ_ESPState.SpectatorESP end
        return KUBZZ_ESPState.SurvivorESP
    end

    local function KUBZZ_PlayerColor(player)
        local role = KUBZZ_GetRole(player)
        if role == "Killer" then return KUBZZ_ESPState.KillerColor end
        if role == "Spectator" then return KUBZZ_ESPState.SpectatorColor end
        return KUBZZ_ESPState.SurvivorColor
    end

    getgenv().KUBZZ_VD_VisualESP_HasPlayerText = function(player)
        if not player or player == LP then return false end
        return KUBZZ_ESPState.PlayerMasterESP
            and KUBZZ_PlayerRoleEnabled(player)
            and (KUBZZ_ESPState.Nametags or KUBZZ_ESPState.DistanceESP)
    end

    local function KUBZZ_EnsureHighlight(name, adornee, color, isPlayer)
        if not (adornee and KUBZZ_Alive(adornee)) then return nil end
        local folder = KUBZZ_GetESPFolder()
        KUBZZ_ClearPrefix(name, name)
        ...
        if isPlayer then
            hl.FillTransparency = KUBZZ_ESPState.ESPFillTransparency
            hl.OutlineTransparency = KUBZZ_ESPState.ESPOutlineTransparency
        else
    ...
    local function KUBZZ_DestroyChild(name)
        local folder = KUBZZ_GetESPFolder()
        local child = folder:FindFirstChild(name)
        if child then child:Destroy() end
    end

    local function KUBZZ_ClearPlayerESP(player)
        if not player or player == LP then return end
        local key = KUBZZ_PlayerKey(player)
        KUBZZ_DestroyChild("KUBZZ_PlayerHL_" .. key)
        KUBZZ_DestroyChild("KUBZZ_PlayerTag_" .. key)
        KUBZZ_DestroyChild("KUBZZ_PlayerItem_" .. key)
    end

    local function KUBZZ_ClearAllPlayerESP()
        for _, player in ipairs(Players:GetPlayers()) do
            if player ~= LP then
                KUBZZ_ClearPlayerESP(player)
            end
        end
    end

    local function KUBZZ_GetSurvivorItem(player)
        local character = player.Character
        if not character then return nil end
        for _, obj in ipairs(character:GetDescendants()) do
            if obj:IsA("Tool") or obj:IsA("Accessory") or obj:IsA("Model") then
                if KUBZZ_DisplayNames[obj.Name] then
                    return obj.Name
                end
            end
        end
    ...
    local function KUBZZ_GetItemImageId(itemName)
    ...
    local function KUBZZ_SetBillboardLine(parent, index, count, data)
        ...
        label.TextSize = KUBZZ_ESPState.ESPTextSize
        label.TextColor3 = data.Color
        label.Text = data.Text
    end

    local function KUBZZ_PruneBillboardLines(parent, count)
    ...
    local function KUBZZ_UpdatePlayerTag(player, character, head, color)
        local key = KUBZZ_PlayerKey(player)
        local tagName = "KUBZZ_PlayerTag_" .. key
        local folder = KUBZZ_GetESPFolder()
        KUBZZ_ClearPrefix("KUBZZ_PlayerTag_" .. key, tagName)

        if not KUBZZ_ValidPart(head) then
            KUBZZ_DestroyChild(tagName)
            return
        end
        ...
        local distanceText = ""
        if KUBZZ_ESPState.DistanceESP and root and targetRoot then
            distanceText = "[" .. tostring(math.floor((root.Position - targetRoot.Position).Magnitude)) .. "m]"
        end

        local nameText = KUBZZ_ESPState.Nametags and player.Name or ""
        local mainLine = ""
        ...
        for i, data in ipairs(lines) do
            KUBZZ_SetBillboardLine(tag, i, #lines, data)
        end
        KUBZZ_PruneBillboardLines(tag, #lines)
    end

    local function KUBZZ_UpdatePlayerItemIcon(player, torso)
        local key = KUBZZ_PlayerKey(player)
        local iconName = "KUBZZ_PlayerItem_" .. key
        local folder = KUBZZ_GetESPFolder()
        KUBZZ_ClearPrefix("KUBZZ_PlayerItem_" .. key, iconName)

        if not KUBZZ_ValidPart(torso) then
            KUBZZ_DestroyChild(iconName)
            return
        end

        local itemName = KUBZZ_GetSurvivorItem(player)
        local imageId = itemName and KUBZZ_GetItemImageId(itemName) or nil
        if not imageId then
            KUBZZ_DestroyChild(iconName)
            return
        end
    ...
    local KUBZZ_ApplyPlayerESP
    KUBZZ_ApplyPlayerESP = function(player)
        if KUBZZ_Dead or not player or player == LP then return end
        local character = player.Character
        if not (character and KUBZZ_Alive(character)) then
            KUBZZ_ClearPlayerESP(player)
            return
        end

        local key = KUBZZ_PlayerKey(player)
        local enabled = KUBZZ_ESPState.PlayerMasterESP and KUBZZ_PlayerRoleEnabled(player)
        if not enabled then
            KUBZZ_ClearPlayerESP(player)
            return
        end

        local color = KUBZZ_PlayerColor(player)
        ...
        KUBZZ_EnsureHighlight("KUBZZ_PlayerHL_" .. key, character, color, true)
        KUBZZ_UpdatePlayerTag(player, character, head, color)

        if KUBZZ_GetRole(player) == "Survivor" and KUBZZ_ESPState.SurvivorItemsESP then
            KUBZZ_UpdatePlayerItemIcon(player, torso)
        else
            KUBZZ_DestroyChild("KUBZZ_PlayerItem_" .. key)
        end
    end

    local function KUBZZ_RefreshAllPlayers()
        for _, player in ipairs(Players:GetPlayers()) do
            if player ~= LP then
                pcall(KUBZZ_ApplyPlayerESP, player)
            end
        end
    end

    local function KUBZZ_StartPlayerLoop()
        if KUBZZ_PlayerLoopThread then return end
        KUBZZ_PlayerLoopThread = task.spawn(function()
            while not KUBZZ_Dead and KUBZZ_ESPState.PlayerMasterESP do
                KUBZZ_RefreshAllPlayers()
                task.wait(0.25)
            end
            KUBZZ_PlayerLoopThread = nil
        end)
    end

    local function KUBZZ_WatchPlayer(player)
        if player == LP then return end
        if KUBZZ_PlayerConns[player] then
            for _, conn in ipairs(KUBZZ_PlayerConns[player]) do
                if conn then pcall(function() conn:Disconnect() end) end
            end
        end

        KUBZZ_PlayerConns[player] = {}
        table.insert(KUBZZ_PlayerConns[player], player.CharacterAdded:Connect(function(char)
            KUBZZ_ClearPlayerESP(player)
            task.delay(0.15, function()
                if not KUBZZ_Dead then pcall(KUBZZ_ApplyPlayerESP, player) end
            end)
        end))
        table.insert(KUBZZ_PlayerConns[player], player.CharacterRemoving:Connect(function()
            KUBZZ_ClearPlayerESP(player)
        end))
        table.insert(KUBZZ_PlayerConns[player], player:GetPropertyChangedSignal("Team"):Connect(function()
            KUBZZ_ClearPlayerESP(player)
            pcall(KUBZZ_ApplyPlayerESP, player)
        end))
    ...
    local function KUBZZ_UnwatchPlayer(player)
        KUBZZ_ClearPlayerESP(player)
        if KUBZZ_PlayerConns[player] then
            for _, conn in ipairs(KUBZZ_PlayerConns[player]) do
                if conn then pcall(function() conn:Disconnect() end) end
        ...
        KUBZZ_PlayerConns[player] = nil
    end

    local function KUBZZ_PickWorldPart(model, cat)
        if not (model and KUBZZ_Alive(model)) then return nil end
        if cat == "Generator" then
            local hitbox = model:FindFirstChild("HitBox", true) or model:FindFirstChild("GeneratorPoint", true)
            if KUBZZ_ValidPart(hitbox) then return hitbox end
        ...
        elseif cat == "Window" then
            ...
            if KUBZZ_ValidPart(vault) then return vault end
        elseif cat == "SCPZombie" then
            local root = model:FindFirstChild("HumanoidRootPart", true)
            if KUBZZ_ValidPart(root) then return root end
            local torso = model:FindFirstChild("UpperTorso", true) or model:FindFirstChild("Torso", true)
            if KUBZZ_ValidPart(torso) then return torso end
            return nil
        end
        return KUBZZ_FirstBasePart(model)
    end

    local function KUBZZ_GeneratorLabel(model)
        local pct = tonumber(model:GetAttribute("RepairProgress")) or 0
        if pct >= 0 and pct <= 1.001 then pct = pct * 100 end
        pct = KUBZZ_Clamp(pct, 0, 100)
        ...
        local hue = KUBZZ_Clamp((pct / 100) * 0.33, 0, 0.33)
        return table.concat(parts, " "), Color3.fromHSV(hue, 1, 1)
    end

    local function KUBZZ_HasBasePart(model)
        if not (model and KUBZZ_Alive(model)) then return false end
        return model:FindFirstChildWhichIsA("BasePart", true) ~= nil
    end

    local function KUBZZ_IsPalletGone(model)
        if not KUBZZ_Alive(model) then return true end
        if not model:IsDescendantOf(Workspace) then return true end
        if KUBZZ_PalletState[model] == "DEST" then return true end
        local ok, destroyed = pcall(function() return model:GetAttribute("Destroyed") end)
        if ok and destroyed == true then return true end
        return not KUBZZ_HasBasePart(model)
    end

    local function KUBZZ_WorldKey(cat, model)
        return "KUBZZ_World_" .. cat .. "_" .. KUBZZ_EspId(model)
    end

    local function KUBZZ_ClearWorldVisual(cat, model)
        if not model then return end
        KUBZZ_DestroyChild(KUBZZ_WorldKey(cat, model) .. "_HL")
        KUBZZ_DestroyChild(KUBZZ_WorldKey(cat, model) .. "_Tag")
    end

    local function KUBZZ_RemoveWorldEntry(cat, model)
        if not KUBZZ_WorldReg[cat] or not KUBZZ_WorldReg[cat][model] then return end
        KUBZZ_ClearWorldVisual(cat, model)
        KUBZZ_WorldReg[cat][model] = nil
    end

    local function KUBZZ_EnsureWorldEntry(cat, model)
        if not KUBZZ_Alive(model) or not KUBZZ_WorldReg[cat] or KUBZZ_WorldReg[cat][model] then return end
        if cat == "Palletwrong" and KUBZZ_IsPalletGone(model) then return end
        local part = KUBZZ_PickWorldPart(model, cat)
        if not KUBZZ_ValidPart(part) then return end
        KUBZZ_WorldReg[cat][model] = { part = part }
    end

    local function KUBZZ_RegisterWorldDescendant(obj)
        if not KUBZZ_Alive(obj) then return end
        local validCats = { Generator = true, Hook = true, Gate = true, Window = true, Palletwrong = true }

        if obj:IsA("Model") then
            if validCats[obj.Name] then
                KUBZZ_EnsureWorldEntry(obj.Name, obj)
                return
            end
            local lower = obj.Name:lower()
            if lower:find("scp") or lower:find("zombie") then
                KUBZZ_EnsureWorldEntry("SCPZombie", obj)
            end
            return
        end
            ...
            while parent and parent ~= Workspace do
                if parent:IsA("Model") then
                    if validCats[parent.Name] then
                        KUBZZ_EnsureWorldEntry(parent.Name, parent)
                        return
                    end
                    local lower = parent.Name:lower()
                    if lower:find("scp") or lower:find("zombie") then
                        KUBZZ_EnsureWorldEntry("SCPZombie", parent)
                        return
                    end
                end
    ...
    local function KUBZZ_UnregisterWorldDescendant(obj)
        if not obj then return end
        local validCats = { Generator = true, Hook = true, Gate = true, Window = true, Palletwrong = true }

        if obj:IsA("Model") then
            if validCats[obj.Name] then
                KUBZZ_RemoveWorldEntry(obj.Name, obj)
                return
            end
            local lower = obj.Name:lower()
            if lower:find("scp") or lower:find("zombie") then
                KUBZZ_RemoveWorldEntry("SCPZombie", obj)
            end
            return
        end

        if obj:IsA("BasePart") then
            for cat, models in pairs(KUBZZ_WorldReg) do
                for model, entry in pairs(models) do
                    if entry.part == obj then
                        KUBZZ_RemoveWorldEntry(cat, model)
                    end
                end
            end
        end
    ...
    local function KUBZZ_AttachESPRoot(root)
        if not root or KUBZZ_MapAdd[root] then return end
        KUBZZ_MapAdd[root] = root.DescendantAdded:Connect(KUBZZ_RegisterWorldDescendant)
        KUBZZ_MapRem[root] = root.DescendantRemoving:Connect(KUBZZ_UnregisterWorldDescendant)
        for _, descendant in ipairs(root:GetDescendants()) do
            KUBZZ_RegisterWorldDescendant(descendant)
        end
    end

    local function KUBZZ_RefreshESPRoots()
        for _, conn in pairs(KUBZZ_MapAdd) do
            if conn then pcall(function() conn:Disconnect() end) end
        end
        for _, conn in pairs(KUBZZ_MapRem) do
            if conn then pcall(function() conn:Disconnect() end) end
        end
        KUBZZ_MapAdd, KUBZZ_MapRem = {}, {}
        ...
        if map then KUBZZ_AttachESPRoot(map) end
        if map1 then KUBZZ_AttachESPRoot(map1) end
    end

    local function KUBZZ_LabelForPallet(model)
        local state = KUBZZ_PalletState[model] or "UP"
    ...
    local function KUBZZ_LabelForWindow(model)
        local state = KUBZZ_WindowState[model] or "READY"
        if state == "BUSY" then return "Window (busy)" end
        return "Window"
    end

    local function KUBZZ_AnyWorldEnabled()
        return KUBZZ_ESPState.WorldMasterESP and (
            KUBZZ_ESPState.GeneratorESP or
            KUBZZ_ESPState.HookESP or
            KUBZZ_ESPState.GateESP or
            KUBZZ_ESPState.WindowESP or
            KUBZZ_ESPState.PalletESP or
            KUBZZ_ESPState.SCPZombieESP
        )
    end

    local function KUBZZ_WorldCategoryData(cat)
        if cat == "Generator" then return KUBZZ_ESPState.GeneratorESP, KUBZZ_ESPState.GeneratorColor end
        if cat == "Hook" then return KUBZZ_ESPState.HookESP, KUBZZ_ESPState.HookColor end
        if cat == "Gate" then return KUBZZ_ESPState.GateESP, KUBZZ_ESPState.GateColor end
        if cat == "Window" then return KUBZZ_ESPState.WindowESP, KUBZZ_ESPState.WindowColor end
        if cat == "Palletwrong" then return KUBZZ_ESPState.PalletESP, KUBZZ_ESPState.PalletColor end
        if cat == "SCPZombie" then return KUBZZ_ESPState.SCPZombieESP, KUBZZ_ESPState.SCPZombieColor end
        return false, Color3.new(1, 1, 1)
    end

    local function KUBZZ_UpdateWorldTag(cat, model, part, color)
        local key = KUBZZ_WorldKey(cat, model)
        local tagName = key .. "_Tag"
        local folder = KUBZZ_GetESPFolder()
        KUBZZ_ClearPrefix(tagName, tagName)

        if not KUBZZ_ValidPart(part) then
            KUBZZ_DestroyChild(tagName)
            return
        end
        ...
        local root = LP.Character and LP.Character:FindFirstChild("HumanoidRootPart")
        local distanceText = ""
        if KUBZZ_ESPState.WorldDistanceESP and root then
            distanceText = "[" .. tostring(math.floor((root.Position - part.Position).Magnitude)) .. "m]"
        end

        local nameText = ""
        local labelColor = color
        if KUBZZ_ESPState.WorldNametags then
            if cat == "Generator" then
                local txt, genColor = KUBZZ_GeneratorLabel(model)
                nameText = txt
                labelColor = genColor
            elseif cat == "Palletwrong" then
                nameText = KUBZZ_LabelForPallet(model)
            elseif cat == "Window" then
                nameText = KUBZZ_LabelForWindow(model)
            elseif cat == "SCPZombie" then
                nameText = model.Name
            else
                nameText = cat
    ...
    local function KUBZZ_ClearAllWorldESP()
        for cat, models in pairs(KUBZZ_WorldReg) do
            for model in pairs(models) do
                KUBZZ_ClearWorldVisual(cat, model)
            end
    ...
    local function KUBZZ_StartWorldLoop()
        if KUBZZ_WorldLoopThread then return end
        KUBZZ_WorldLoopThread = task.spawn(function()
            while not KUBZZ_Dead and KUBZZ_AnyWorldEnabled() do
                for cat, models in pairs(KUBZZ_WorldReg) do
                    local enabled, color = KUBZZ_WorldCategoryData(cat)
                    if enabled and KUBZZ_ESPState.WorldMasterESP then
                        local n = 0
                        for model, entry in pairs(models) do
                            if cat == "Palletwrong" and KUBZZ_IsPalletGone(model) then
                                KUBZZ_RemoveWorldEntry(cat, model)
                            elseif model and KUBZZ_Alive(model) then
                                local part = entry.part
                                if not KUBZZ_ValidPart(part) or (model:IsA("Model") and not part:IsDescendantOf(model)) then
                                    entry.part = KUBZZ_PickWorldPart(model, cat)
                                    part = entry.part
                                end

                                if KUBZZ_ValidPart(part) then
                                    local key = KUBZZ_WorldKey(cat, model)
                                    KUBZZ_EnsureHighlight(key .. "_HL", model, color, false)
                                    KUBZZ_UpdateWorldTag(cat, model, part, color)
                                else
                                    KUBZZ_RemoveWorldEntry(cat, model)
                                end
                            else
            ...
            KUBZZ_WorldLoopThread = nil
        end)
    end

    local function KUBZZ_Selected(selected, name)
    ...
    getgenv().KUBZZ_AddVisualESPControls = function(VisualTabRef)
        if not VisualTabRef or KUBZZ_ControlsAdded then return end
        KUBZZ_ControlsAdded = true
        ...
        settingsSection:AddSlider({
            ...
            Flag = "KUBZZ ESP Fill Transparency",
            Min = 0,
            Max = 1,
            Default = KUBZZ_ESPState.ESPFillTransparency,
            Increment = 0.01,
            Callback = function(value)
                KUBZZ_ESPState.ESPFillTransparency = value
                KUBZZ_RefreshAllPlayers()
            end,
        })
        ...
        settingsSection:AddSlider({
            ...
            Flag = "KUBZZ ESP Outline Transparency",
            Min = 0,
            Max = 1,
            Default = KUBZZ_ESPState.ESPOutlineTransparency,
            Increment = 0.01,
            Callback = function(value)
                KUBZZ_ESPState.ESPOutlineTransparency = value
                KUBZZ_RefreshAllPlayers()
            end,
        })
        ...
        settingsSection:AddSlider({
            Name = "ESP Text Size",
            Flag = "KUBZZ ESP Text Size",
            Min = 8,
            Max = 22,
            Default = KUBZZ_ESPState.ESPTextSize,
            Increment = 1,
            Callback = function(value)
                KUBZZ_ESPState.ESPTextSize = value
                KUBZZ_RefreshAllPlayers()
            end,
        })
        ...
        playerSection:AddToggle({
            ...
            Flag = "KUBZZ Enable Player ESP",
            Default = false,
            Callback = function(state)
                KUBZZ_ESPState.PlayerMasterESP = state
                if state then
                    KUBZZ_StartPlayerLoop()
                    KUBZZ_RefreshAllPlayers()
                else
                    KUBZZ_ClearAllPlayerESP()
                end
            end,
        })
        ...
        playerSection:AddDropdown({
            ...
            Flag = "KUBZZ Select Player ESP",
            Values = { "Survivor ESP", "Killer ESP", "Spectator ESP", "Survivor Items ESP" },
            Multi = true,
            AllowNone = true,
            Default = {},
            Callback = function(selected)
                KUBZZ_ESPState.SurvivorESP = KUBZZ_Selected(selected, "Survivor ESP")
                KUBZZ_ESPState.KillerESP = KUBZZ_Selected(selected, "Killer ESP")
                KUBZZ_ESPState.SpectatorESP = KUBZZ_Selected(selected, "Spectator ESP")
                KUBZZ_ESPState.SurvivorItemsESP = KUBZZ_Selected(selected, "Survivor Items ESP")

                if KUBZZ_ESPState.PlayerMasterESP then
                    KUBZZ_StartPlayerLoop()
                    KUBZZ_RefreshAllPlayers()
                else
                    KUBZZ_ClearAllPlayerESP()
        ...
        playerSection:AddToggle({
            ...
            Flag = "KUBZZ Player Nametags",
            Default = false,
            Callback = function(state)
                KUBZZ_ESPState.Nametags = state
                if KUBZZ_ESPState.PlayerMasterESP then
                    KUBZZ_StartPlayerLoop()
                    KUBZZ_RefreshAllPlayers()
                else
        ...
        playerSection:AddToggle({
            ...
            Flag = "KUBZZ Player Distance ESP",
            Default = false,
            Callback = function(state)
                KUBZZ_ESPState.DistanceESP = state
                if KUBZZ_ESPState.PlayerMasterESP then
                    KUBZZ_StartPlayerLoop()
                    KUBZZ_RefreshAllPlayers()
                else
        ...
        playerSection:AddColorPicker({ Name = "Survivor Color", Flag = "KUBZZ Survivor Color", Default = KUBZZ_ESPState.SurvivorColor, Callback = function(color) KUBZZ_ESPState.SurvivorColor = color; KUBZZ_RefreshAllPlayers() end })
        playerSection:AddColorPicker({ Name = "Killer Color", Flag = "KUBZZ Killer Color", Default = KUBZZ_ESPState.KillerColor, Callback = function(color) KUBZZ_ESPState.KillerColor = color; KUBZZ_RefreshAllPlayers() end })
        playerSection:AddColorPicker({ Name = "Spectator Color", Flag = "KUBZZ Spectator Color", Default = KUBZZ_ESPState.SpectatorColor, Callback = function(color) KUBZZ_ESPState.SpectatorColor = color; KUBZZ_RefreshAllPlayers() end })
        ...
        worldSection:AddToggle({
            ...
            Flag = "KUBZZ Enable World ESP",
            Default = false,
            Callback = function(state)
                KUBZZ_ESPState.WorldMasterESP = state
                if state then
                    KUBZZ_RefreshESPRoots()
                    if KUBZZ_AnyWorldEnabled() then KUBZZ_StartWorldLoop() end
                else
                    KUBZZ_ClearAllWorldESP()
                end
            end,
        })
        ...
        worldSection:AddDropdown({
            ...
            Flag = "KUBZZ Select World Objects",
            Values = { "Generators", "Hooks", "Gates", "Windows", "Pallets", "SCP / Zombie" },
            Multi = true,
            AllowNone = true,
            Default = {},
            Callback = function(selected)
                KUBZZ_ESPState.GeneratorESP = KUBZZ_Selected(selected, "Generators")
                KUBZZ_ESPState.HookESP = KUBZZ_Selected(selected, "Hooks")
                KUBZZ_ESPState.GateESP = KUBZZ_Selected(selected, "Gates")
                KUBZZ_ESPState.WindowESP = KUBZZ_Selected(selected, "Windows")
                KUBZZ_ESPState.PalletESP = KUBZZ_Selected(selected, "Pallets")
                KUBZZ_ESPState.SCPZombieESP = KUBZZ_Selected(selected, "SCP / Zombie")

                if KUBZZ_ESPState.WorldMasterESP and KUBZZ_AnyWorldEnabled() then
                    KUBZZ_RefreshESPRoots()
                    KUBZZ_StartWorldLoop()
                else
                    KUBZZ_ClearAllWorldESP()
                end
            end,
        ...
        worldSection:AddToggle({
            Name = "World Nametags",
            Flag = "KUBZZ World Nametags",
            Default = false,
            Callback = function(state)
                KUBZZ_ESPState.WorldNametags = state
                if KUBZZ_ESPState.WorldMasterESP and KUBZZ_AnyWorldEnabled() then KUBZZ_StartWorldLoop() else KUBZZ_ClearAllWorldESP() end
            end,
        })

        worldSection:AddToggle({
            Name = "World Distance ESP",
            Flag = "KUBZZ World Distance ESP",
            Default = false,
            Callback = function(state)
                KUBZZ_ESPState.WorldDistanceESP = state
                if KUBZZ_ESPState.WorldMasterESP and KUBZZ_AnyWorldEnabled() then KUBZZ_StartWorldLoop() else KUBZZ_ClearAllWorldESP() end
            end,
        })

        pcall(function() worldSection:AddDivider({ Text = "Colors" }) end)
        worldSection:AddColorPicker({ Name = "Generator Color", Flag = "KUBZZ Generator Color", Default = KUBZZ_ESPState.GeneratorColor, Callback = function(color) KUBZZ_ESPState.GeneratorColor = color end })
        worldSection:AddColorPicker({ Name = "Hook Color", Flag = "KUBZZ Hook Color", Default = KUBZZ_ESPState.HookColor, Callback = function(color) KUBZZ_ESPState.HookColor = color end })
        worldSection:AddColorPicker({ Name = "Gate Color", Flag = "KUBZZ Gate Color", Default = KUBZZ_ESPState.GateColor, Callback = function(color) KUBZZ_ESPState.GateColor = color end })
        worldSection:AddColorPicker({ Name = "Window Color", Flag = "KUBZZ Window Color", Default = KUBZZ_ESPState.WindowColor, Callback = function(color) KUBZZ_ESPState.WindowColor = color end })
        worldSection:AddColorPicker({ Name = "Pallet Color", Flag = "KUBZZ Pallet Color", Default = KUBZZ_ESPState.PalletColor, Callback = function(color) KUBZZ_ESPState.PalletColor = color end })
        worldSection:AddColorPicker({ Name = "SCP / Zombie Color", Flag = "KUBZZ SCP Zombie Color", Default = KUBZZ_ESPState.SCPZombieColor, Callback = function(color) KUBZZ_ESPState.SCPZombieColor = color end })
    end

    for _, player in ipairs(Players:GetPlayers()) do
        KUBZZ_WatchPlayer(player)
    end

    table.insert(KUBZZ_Connections, Players.PlayerAdded:Connect(KUBZZ_WatchPlayer))
    table.insert(KUBZZ_Connections, Players.PlayerRemoving:Connect(KUBZZ_UnwatchPlayer))
    table.insert(KUBZZ_Connections, Workspace.ChildAdded:Connect(function(child)
        if child.Name == "Map" or child.Name == "Map1" then
            KUBZZ_AttachESPRoot(child)
            if KUBZZ_ESPState.WorldMasterESP and KUBZZ_AnyWorldEnabled() then KUBZZ_StartWorldLoop() end
        end
    end))
    table.insert(KUBZZ_Connections, Workspace.ChildRemoved:Connect(function(child)
        if child.Name == "Map" or child.Name == "Map1" then
            KUBZZ_RefreshESPRoots()
        end
    end))
    ...
    getgenv().KUBZZ_VD_VisualESP_Cleanup = function()
        KUBZZ_Dead = true
        KUBZZ_ClearAllPlayerESP()
        KUBZZ_ClearAllWorldESP()

        for _, conn in ipairs(KUBZZ_Connections) do
            if conn then pcall(function() conn:Disconnect() end) end
        end
        for _, conns in pairs(KUBZZ_PlayerConns) do
        ...
        if KUBZZ_ESPFolder and KUBZZ_ESPFolder.Parent then
            KUBZZ_ESPFolder:Destroy()
        end
    end

    KUBZZ_SafeNotify("Visual ESP", "Highlight ESP V2 loaded. Anti double nametag aktif.", 3)
end
...
task.spawn(function()
    while not VD.Destroyed do
        if VD.Fullbright then
            local weatherTheme = VD.VIS_WeatherTheme and KUBZZ_WeatherPresets[VD.VIS_WeatherTheme]
            ...
            if VD.VIS_WeatherTheme and VD.VIS_WeatherTheme ~= "Default" and KUBZZ_WeatherPresets[VD.VIS_WeatherTheme] then
                local theme = KUBZZ_WeatherPresets[VD.VIS_WeatherTheme]
                if theme.Lighting then
                    for k, v in pairs(theme.Lighting) do
                        pcall(function() Lighting[k] = v end)
                    end
...
local function KUBZZ_ToFGetEvent()
...
local function KUBZZ_ToFGetGunObject()
    local char = LocalPlayer.Character
...
local function KUBZZ_ToFIsTargetVisible(originPos, targetPos, targetCharacter)
    ...
    if KUBZZ_ToFState.LaserBeam then table.insert(excludeList, KUBZZ_ToFState.LaserBeam) end
...
local function KUBZZ_ToFGetSCPs()
    if tick() - KUBZZ_ToFState.SCPCacheTimer < 0.5 then
        return KUBZZ_ToFState.SCPCache
    end
    ...
    KUBZZ_ToFState.SCPCache = newTargets
    KUBZZ_ToFState.SCPCacheTimer = tick()
    return KUBZZ_ToFState.SCPCache
end

local function KUBZZ_ToFGetTargetPosition()
    local gunObj = KUBZZ_ToFGetGunObject()
    local char = LocalPlayer.Character
    ...
    local function predictTarget(torso, targetCharacter)
        ...
        if VD.TOF_WallCheck and not KUBZZ_ToFIsTargetVisible(originPos, targetPos, targetCharacter) then
    ...
    elseif targetMode == "Zombie" then
        ...
        for _, root in ipairs(KUBZZ_ToFGetSCPs()) do
...
local function KUBZZ_ToFUpdateLaser(originPos, targetPos)
    if not KUBZZ_ToFState.LaserBeam then
        ...
        KUBZZ_ToFState.LaserBeam = laser
    end

    local dist = (targetPos - originPos).Magnitude
    KUBZZ_ToFState.LaserBeam.Size = Vector3.new(0.05, 0.05, dist)
    KUBZZ_ToFState.LaserBeam.CFrame = CFrame.new((originPos + targetPos) / 2, targetPos)
    KUBZZ_ToFState.LaserBeam.Transparency = 0
end

local function KUBZZ_ToFClearLaser()
    if KUBZZ_ToFState.LaserBeam then
        pcall(function() KUBZZ_ToFState.LaserBeam:Destroy() end)
        KUBZZ_ToFState.LaserBeam = nil
    end
end
...
local function KUBZZ_ToFGetMobileShootButton()
...
local function KUBZZ_ToFIsTouchOnShootButton(input)
    local shootButton = KUBZZ_ToFGetMobileShootButton()
...
local function KUBZZ_ToFDoShoot()
    ...
    local targetDirection, gunObject, originPos, targetPos = KUBZZ_ToFGetTargetPosition()
    if not (targetDirection and gunObject and targetPos and originPos) then return end

    local tofEvent = KUBZZ_ToFGetEvent()
...
local KUBZZ_ToFModeButtons = {}
local function KUBZZ_ToFRefreshTargetButtons()
    local modes = {
    ...
    for modeName, btn in pairs(KUBZZ_ToFModeButtons) do
...
local function KUBZZ_ToFSetTargetMode(modeName, notify)
    if modeName ~= "Killer" and modeName ~= "Survivors" and modeName ~= "Zombie" then return end
    VD.TOF_TargetMode = modeName
    KUBZZ_ToFRefreshTargetButtons()
    if notify then VD_Notify("Target Mode", modeName, 1) end
end

local function KUBZZ_ToFCreateTargetSelectorUI()
    local parent = GetSafeGuiParent()
    if not parent then return end
    if KUBZZ_ToFState.TargetGui and KUBZZ_ToFState.TargetGui.Parent then return end
    ...
    frame.Position = KUBZZ_ToFState.SavedUIPos
    ...
    for i, mode in ipairs(modes) do
        ...
        btn.MouseButton1Click:Connect(function()
            KUBZZ_ToFSetTargetMode(mode.Internal, false)
        end)
        ...
        KUBZZ_ToFModeButtons[mode.Internal] = btn
    end
    KUBZZ_ToFRefreshTargetButtons()
    ...
    UserInputService.InputChanged:Connect(function(input)
        ...
        if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
            ...
            KUBZZ_ToFState.SavedUIPos = newPos
        end
    end)

    KUBZZ_ToFState.TargetGui = gui
end

local function KUBZZ_ToFDestroyTargetSelectorUI()
    if KUBZZ_ToFState.TargetGui then
        pcall(function() KUBZZ_ToFState.TargetGui:Destroy() end)
        KUBZZ_ToFState.TargetGui = nil
    end
    KUBZZ_ToFModeButtons = {}
end

local function KUBZZ_ToFStartConnection()
    if KUBZZ_ToFState.Connection then return end
    KUBZZ_ToFState.Connection = RunService.Heartbeat:Connect(function()
        if not VD.TOF_SilentAim or not KUBZZ_ToFState.IsAiming then
            if KUBZZ_ToFState.LaserBeam then KUBZZ_ToFState.LaserBeam.Transparency = 1 end
            return
        end

        local _, _, originPos, targetPos = KUBZZ_ToFGetTargetPosition()
        if originPos and targetPos then
            ...
            if VD.TOF_Laser then
                KUBZZ_ToFUpdateLaser(originPos, targetPos)
            elseif KUBZZ_ToFState.LaserBeam then
                KUBZZ_ToFState.LaserBeam.Transparency = 1
            end
        elseif KUBZZ_ToFState.LaserBeam then
            KUBZZ_ToFState.LaserBeam.Transparency = 1
        end
...
local function KUBZZ_ToFStopConnection()
    if KUBZZ_ToFState.Connection then
        pcall(function() KUBZZ_ToFState.Connection:Disconnect() end)
        KUBZZ_ToFState.Connection = nil
    end
    KUBZZ_ToFState.IsAiming = false
    KUBZZ_ToFClearLaser()
end

local function KUBZZ_ToFDisconnectInputs()
    if KUBZZ_ToFState.InputBegan then pcall(function() KUBZZ_ToFState.InputBegan:Disconnect() end) end
    if KUBZZ_ToFState.InputEnded then pcall(function() KUBZZ_ToFState.InputEnded:Disconnect() end) end
    KUBZZ_ToFState.InputBegan = nil
    KUBZZ_ToFState.InputEnded = nil
end

local KUBZZ_SetToFSilentAim

local function KUBZZ_ToFEnsureInputs()
    if not KUBZZ_ToFState.InputBegan then
        KUBZZ_ToFState.InputBegan = UserInputService.InputBegan:Connect(function(input, gameProcessed)
            if gameProcessed then return end

            local keyCode = KUBZZ_ToFKeyCodes[VD.TOF_Key or "None"]
            if keyCode and input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode == keyCode then
                KUBZZ_SetToFSilentAim(not VD.TOF_SilentAim)
                return
            end

            if not VD.TOF_SilentAim then return end
            if input.UserInputType == Enum.UserInputType.MouseButton1
            or (input.UserInputType == Enum.UserInputType.Touch and KUBZZ_ToFIsTouchOnShootButton(input)) then
                KUBZZ_ToFState.IsAiming = true
                if input.UserInputType == Enum.UserInputType.Touch then
                    KUBZZ_ToFState.TouchInput = input
                end
                KUBZZ_ToFDoShoot()
                return
            end
            ...
            if input.UserInputType == Enum.UserInputType.Keyboard then
                if input.KeyCode == Enum.KeyCode.K then
                    KUBZZ_ToFSetTargetMode("Killer", true)
                elseif input.KeyCode == Enum.KeyCode.J then
                    KUBZZ_ToFSetTargetMode("Survivors", true)
                elseif input.KeyCode == Enum.KeyCode.L then
                    KUBZZ_ToFSetTargetMode("Zombie", true)
                end
            end
        end)
    end
    if not KUBZZ_ToFState.InputEnded then
        KUBZZ_ToFState.InputEnded = UserInputService.InputEnded:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1
            or (input.UserInputType == Enum.UserInputType.Touch and input == KUBZZ_ToFState.TouchInput) then
                KUBZZ_ToFState.IsAiming = false
                if input == KUBZZ_ToFState.TouchInput then KUBZZ_ToFState.TouchInput = nil end
                if KUBZZ_ToFState.LaserBeam then KUBZZ_ToFState.LaserBeam.Transparency = 1 end
            end
        end)
    end
...
KUBZZ_SetToFSilentAim = function(enabled)
    VD.TOF_SilentAim = enabled and true or false
    KUBZZ_ToFEnsureInputs()
    if VD.TOF_SilentAim then
        KUBZZ_ToFCreateTargetSelectorUI()
        KUBZZ_ToFStartConnection()
    else
        KUBZZ_ToFDestroyTargetSelectorUI()
        KUBZZ_ToFStopConnection()
    end
end

KUBZZ_ToFEnsureInputs()
getgenv().KUBZZ_SetToFSilentAim = KUBZZ_SetToFSilentAim
getgenv().KUBZZ_ToFClearLaser = KUBZZ_ToFClearLaser
getgenv().KUBZZ_ToFSetTargetMode = KUBZZ_ToFSetTargetMode
end)();
...
-- =====================================================
(function()
local KUBZZ_HideSurvivorIconState = {
    Connection = nil,
    Originals = {},
}

local KUBZZ_HideSurvivorIconImage = "rbxassetid://95802749943553"
local KUBZZ_HideSurvivorIconText = "NxH"

local function KUBZZ_GetSurvivorSlots()
    local slots = {}
    local playerGui = LocalPlayer:FindFirstChild("PlayerGui")
...
local function KUBZZ_ApplyHideSurvivorIcon()
    for _, slot in ipairs(KUBZZ_GetSurvivorSlots()) do
        local imageLabel = slot.ImageLabel
        if imageLabel and imageLabel:IsA("ImageLabel") then
            if not KUBZZ_HideSurvivorIconState.Originals[imageLabel] then
                KUBZZ_HideSurvivorIconState.Originals[imageLabel] = {
            ...
            imageLabel.Image = KUBZZ_HideSurvivorIconImage
        ...
        if textLabel and textLabel:IsA("TextLabel") then
            if not KUBZZ_HideSurvivorIconState.Originals[textLabel] then
                KUBZZ_HideSurvivorIconState.Originals[textLabel] = {
                    Text = textLabel.Text,
            ...
            textLabel.Text = KUBZZ_HideSurvivorIconText
...
local function KUBZZ_RestoreSurvivorIcons()
    for object, original in pairs(KUBZZ_HideSurvivorIconState.Originals) do
    ...
    KUBZZ_HideSurvivorIconState.Originals = {}
end

local function KUBZZ_SetHideSurvivorIcon(enabled)
    VD.VIS_HideSurvivorIcon = enabled and true or false

    if VD.VIS_HideSurvivorIcon then
        KUBZZ_ApplyHideSurvivorIcon()
        if not KUBZZ_HideSurvivorIconState.Connection then
            KUBZZ_HideSurvivorIconState.Connection = RunService.Heartbeat:Connect(function()
                if VD.VIS_HideSurvivorIcon then
                    KUBZZ_ApplyHideSurvivorIcon()
                end
            end)
        ...
        if KUBZZ_HideSurvivorIconState.Connection then
            pcall(function() KUBZZ_HideSurvivorIconState.Connection:Disconnect() end)
            KUBZZ_HideSurvivorIconState.Connection = nil
        end
        KUBZZ_RestoreSurvivorIcons()
    end
end

getgenv().KUBZZ_SetHideSurvivorIcon = KUBZZ_SetHideSurvivorIcon
end)();
...
-- =====================================================
(function()
local KUBZZ_HookCounterState = {
    Connection = nil,
}

local function KUBZZ_UpdateHookCounter(enabled)
    local playerGui = LocalPlayer:FindFirstChild("PlayerGui")
            ...
            if frame then
                for i = 1, 5 do
                    ...
                    if imageLabel and textLabel then
                        ...
                        local labelName = "KUBZZ_CustomHookCounter"
...
local function KUBZZ_SetShowHookCounter(enabled)
    VD.VIS_ShowHookCounter = enabled and true or false

    if VD.VIS_ShowHookCounter then
        KUBZZ_UpdateHookCounter(true)
        if not KUBZZ_HookCounterState.Connection then
            KUBZZ_HookCounterState.Connection = RunService.Heartbeat:Connect(function()
                if VD.VIS_ShowHookCounter then
                    KUBZZ_UpdateHookCounter(true)
                end
            end)
        ...
        if KUBZZ_HookCounterState.Connection then
            pcall(function() KUBZZ_HookCounterState.Connection:Disconnect() end)
            KUBZZ_HookCounterState.Connection = nil
        end
        KUBZZ_UpdateHookCounter(false)
                ...
                if gui:IsA("ScreenGui") and gui.Name:match("%-mob$") then
                    ...
                    if frame then
                        for i = 1, 5 do
                            local survivorFrame = frame:FindFirstChild("Survivor" .. i)
                            local imageLabel = survivorFrame and survivorFrame:FindFirstChild("ImageLabel")
                            customLabel = imageLabel and imageLabel:FindFirstChild("KUBZZ_CustomHookCounter")
                            if customLabel then
...
getgenv().KUBZZ_SetShowHookCounter = KUBZZ_SetShowHookCounter
end)();
...
local function KUBZZ_GetPingValue()
...
local function KUBZZ_CreatePingFPSGui()
    local parent = GetSafeGuiParent()
    if not parent then return nil end

    local old = parent:FindFirstChild("KUBZZ_PingFPSGui")
    if old then pcall(function() old:Destroy() end) end

    local sg = Instance.new("ScreenGui")
    sg.Name = "KUBZZ_PingFPSGui"
...
local function KUBZZ_SetShowPingFPS(enabled)
    VD.VIS_ShowPingFPS = enabled and true or false

    if VD.VIS_ShowPingFPS then
        KUBZZ_PingFPSState.Gui = KUBZZ_PingFPSState.Gui or KUBZZ_CreatePingFPSGui()
        KUBZZ_PingFPSState.Frames = 0
        KUBZZ_PingFPSState.LastUpdate = tick()

        if not KUBZZ_PingFPSState.Connection then
            KUBZZ_PingFPSState.Connection = RunService.RenderStepped:Connect(function()
                if not VD.VIS_ShowPingFPS then return end

                KUBZZ_PingFPSState.Frames = KUBZZ_PingFPSState.Frames + 1
                local now = tick()
                if now - KUBZZ_PingFPSState.LastUpdate < 0.5 then return end

                local fps = math.floor(KUBZZ_PingFPSState.Frames / (now - KUBZZ_PingFPSState.LastUpdate) + 0.5)
                local ping = KUBZZ_GetPingValue()
                KUBZZ_PingFPSState.Frames = 0
                KUBZZ_PingFPSState.LastUpdate = now

                if not (KUBZZ_PingFPSState.Gui and KUBZZ_PingFPSState.Gui.Parent) then
                    KUBZZ_PingFPSState.Gui = KUBZZ_CreatePingFPSGui()
                end

                local label = KUBZZ_PingFPSState.Gui and KUBZZ_PingFPSState.Gui:FindFirstChild("PingFPSLabel", true)
                if label then
        ...
        if KUBZZ_PingFPSState.Connection then
            pcall(function() KUBZZ_PingFPSState.Connection:Disconnect() end)
            KUBZZ_PingFPSState.Connection = nil
        end
        if KUBZZ_PingFPSState.Gui then
            pcall(function() KUBZZ_PingFPSState.Gui:Destroy() end)
            KUBZZ_PingFPSState.Gui = nil
        end
    end
end

getgenv().KUBZZ_SetShowPingFPS = KUBZZ_SetShowPingFPS
end)();
...
-- =====================================================
(function()
local KUBZZ_FlashlightAimState = {
    Connection = nil,
    LaserBeam = nil,
    FlashlightPart = nil,
    Active = false,
...
local function KUBZZ_GetFlashlightActivateRemote()
    local remotes = ReplicatedStorage:FindFirstChild("Remotes")
    local items = remotes and remotes:FindFirstChild("Items")
...
local function KUBZZ_GetFlashlightTargetPart(char)
    if not char then return nil end
...
local function KUBZZ_IsAliveCharacter(char)
...
local function KUBZZ_GetFlashlightTarget()
    local localChar = LocalPlayer.Character
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character and KUBZZ_IsAliveCharacter(player.Character) then
            local isKiller = player.Team and player.Team.Name == "Killer"
            if isKiller then
                local part = KUBZZ_GetFlashlightTargetPart(player.Character)
                if part then
...
local function KUBZZ_ClearFlashlightLaser()
    if KUBZZ_FlashlightAimState.LaserBeam then
        pcall(function() KUBZZ_FlashlightAimState.LaserBeam:Destroy() end)
        KUBZZ_FlashlightAimState.LaserBeam = nil
    end
end

local function KUBZZ_GetFlashlightOrigin(cam)
    local source = KUBZZ_FlashlightAimState.FlashlightPart
...
local function KUBZZ_UpdateFlashlightLaser(originPos, targetPos)
    if not KUBZZ_FlashlightAimState.LaserBeam then
        local laser = Instance.new("Part")
        ...
        laser.Parent = workspace
        KUBZZ_FlashlightAimState.LaserBeam = laser
    end

    local dist = (targetPos - originPos).Magnitude
    if dist < 0.1 then return end

    local laser = KUBZZ_FlashlightAimState.LaserBeam
    laser.Size = Vector3.new(0.16, 0.16, dist)
    laser.CFrame = CFrame.new((originPos + targetPos) / 2, targetPos)
    laser.Transparency = 0
end

local function KUBZZ_FlashlightAimStep()
    if false then
        if KUBZZ_FlashlightAimState.LaserBeam then
            KUBZZ_FlashlightAimState.LaserBeam.Transparency = 1
        end
        return
    end

    if not (VD.FLASH_SilentAim and KUBZZ_FlashlightAimState.Active) then
        if KUBZZ_FlashlightAimState.LaserBeam then
            KUBZZ_FlashlightAimState.LaserBeam.Transparency = 1
        end
        return
    ...
    local cam = workspace.CurrentCamera
    local targetPart = KUBZZ_GetFlashlightTarget()
    ...
    local originPos = KUBZZ_GetFlashlightOrigin(cam)

    if VD.FLASH_Laser and originPos then
        KUBZZ_UpdateFlashlightLaser(originPos, targetPos)
    elseif KUBZZ_FlashlightAimState.LaserBeam then
        KUBZZ_FlashlightAimState.LaserBeam.Transparency = 1
    end
...
local function KUBZZ_StartFlashlightSilentAim()
    getgenv().KUBZZ_FlashlightActivateRemote = KUBZZ_GetFlashlightActivateRemote()
    if KUBZZ_FlashlightAimState.Connection then return end
    KUBZZ_FlashlightAimState.Connection = RunService.RenderStepped:Connect(KUBZZ_FlashlightAimStep)
end

local function KUBZZ_StopFlashlightSilentAim()
    KUBZZ_FlashlightAimState.Active = false
    KUBZZ_FlashlightAimState.FlashlightPart = nil
    KUBZZ_ClearFlashlightLaser()
    if KUBZZ_FlashlightAimState.Connection then
        pcall(function() KUBZZ_FlashlightAimState.Connection:Disconnect() end)
        KUBZZ_FlashlightAimState.Connection = nil
    end
end

local function KUBZZ_SetFlashlightSilentAim(enabled)
    if enabled and false then
        VD.FLASH_SilentAim = false
        KUBZZ_StopFlashlightSilentAim()
        return
    end

    VD.FLASH_SilentAim = enabled and true or false
    if VD.FLASH_SilentAim then
        KUBZZ_StartFlashlightSilentAim()
    else
        KUBZZ_StopFlashlightSilentAim()
    end
end

getgenv().KUBZZ_SetFlashlightSilentAim = KUBZZ_SetFlashlightSilentAim
getgenv().KUBZZ_ClearFlashlightLaser = KUBZZ_ClearFlashlightLaser
getgenv().KUBZZ_SetFlashlightAimActive = function(active, flashlightPart)
    KUBZZ_FlashlightAimState.Active = active and true or false
    if KUBZZ_FlashlightAimState.Active and flashlightPart then
        KUBZZ_FlashlightAimState.FlashlightPart = flashlightPart
    elseif not KUBZZ_FlashlightAimState.Active then
        KUBZZ_FlashlightAimState.FlashlightPart = nil
    end
    if not KUBZZ_FlashlightAimState.Active and KUBZZ_FlashlightAimState.LaserBeam then
        KUBZZ_FlashlightAimState.LaserBeam.Transparency = 1
    end
end
getgenv().KUBZZ_FlashlightActivateRemote = KUBZZ_GetFlashlightActivateRemote()
...
VD_ParryRange.Name = "KUBZZ_ParryRange"
    ...
    if VeilState.chargingSpear and VeilConfig.Enabled and isSpearMode then
        ...
        if target and target.Part and target.Part.Parent then
            ...
            if VeilConfig.ShowTargetLaser then
                if not getgenv().KUBZZ_SpearLaserPart then
                    local laser = Instance.new("Part")
                    ...
                    laser.Transparency = 0
                    laser.Parent = workspace
                    getgenv().KUBZZ_SpearLaserPart = laser
                end
                ...
                if originPart then
                    ...
                    local dist = (targetPos - originPos).Magnitude
                    if dist > 0.1 then
                        local laser = getgenv().KUBZZ_SpearLaserPart
                        laser.Size = Vector3.new(0.16, 0.16, dist)
                        laser.CFrame = CFrame.new((originPos + targetPos) / 2, targetPos)
                ...
                if getgenv().KUBZZ_SpearLaserPart then getgenv().KUBZZ_SpearLaserPart.Transparency = 1 end
            end
        else
...
local KUBZZ_MainInfoPanel = {
    Widgets = {},
    Texts = {},
}

function KUBZZ_InfoPlainText(text)
...
function KUBZZ_UpdateInfoWidget(widget, text)
...
function KUBZZ_SetMainInfoPanelText(key, title, text)
    local value = tostring(title or key) .. "\n" .. KUBZZ_InfoPlainText(text)
    KUBZZ_MainInfoPanel.Texts[key] = value
    KUBZZ_UpdateInfoWidget(KUBZZ_MainInfoPanel.Widgets[key], value)
end

function KUBZZ_RegisterMainInfoWidget(key, widget)
    KUBZZ_MainInfoPanel.Widgets[key] = widget
    if KUBZZ_MainInfoPanel.Texts[key] then
        KUBZZ_UpdateInfoWidget(widget, KUBZZ_MainInfoPanel.Texts[key])
    end
end

function KUBZZ_AddMainInfoLine(section, key, title, defaultText)
    local defaultValue = tostring(title) .. "\n" .. tostring(defaultText or "Off")
    KUBZZ_MainInfoPanel.Texts[key] = KUBZZ_MainInfoPanel.Texts[key] or defaultValue
    ...
    if ok and widget then return KUBZZ_RegisterMainInfoWidget(key, widget) end
...
do -- Player Tab
    ...
    movSection:AddToggle({ Default = false, Name = "Invisible Not Visual", Locked = false, Flag = "Invisible Not Visual", Callback = function(v) 
...
do -- ESP Tab
    ...
    pcall(function()
        if getgenv().KUBZZ_AddVisualESPControls then
            getgenv().KUBZZ_AddVisualESPControls(VisualTab)
        end
    end)
...
do -- Aim Tab
    ...
    spearSection:AddToggle({ Default = false, Name = "Silent Aim Spear (Veil)", Locked = false, Flag = "Silent Aim Spear (Veil)", Callback = function(v)
    ...
    flaskSection:AddToggle({ Default = false, Name = "Silent Aim Flask (Cure)", Locked = false, Flag = "Silent Aim Flask (Cure)", Callback = function(v)
        VD.KILLER_SilentAimFlask = v
    end })
    flaskSection:AddToggle({ Default = false, Name = "Flask Laser (Cure)", Locked = false, Flag = "Flask Laser (Cure)", Callback = function(v)
        VD.KILLER_FlaskLaser = v
        if v then
            pcall(KUBZZ_StartCureFlaskLaser)
        else
            if getgenv().KUBZZ_CureFlaskLaserThread then
                getgenv().KUBZZ_CureFlaskLaserThread:Disconnect()
                getgenv().KUBZZ_CureFlaskLaserThread = nil
            end
            if getgenv().KUBZZ_CureFlaskLaserPart then
                pcall(function() getgenv().KUBZZ_CureFlaskLaserPart:Destroy() end)
                getgenv().KUBZZ_CureFlaskLaserPart = nil
            end
        end
    end })
    ...
    tofSection:AddToggle({
        ...
        Callback = function(v)
            if getgenv().KUBZZ_SetToFSilentAim then
                getgenv().KUBZZ_SetToFSilentAim(v)
            end
        end
    })
    tofSection:AddToggle({
        ...
        Callback = function(v)
            VD.TOF_Laser = v
            if not v and getgenv().KUBZZ_ToFClearLaser then
                getgenv().KUBZZ_ToFClearLaser()
            end
        end
    })
    tofSection:AddToggle({
    ...
    tofSection:AddDropdown({
        ...
        Callback = function(v)
            if type(v) == "table" then v = v[1] end
            if getgenv().KUBZZ_ToFSetTargetMode then
                getgenv().KUBZZ_ToFSetTargetMode(v or "Killer", false)
            else
    ...
    flashlightSection:AddToggle({
        ...
        Callback = function(v)
            if getgenv().KUBZZ_SetFlashlightSilentAim then
                getgenv().KUBZZ_SetFlashlightSilentAim(v)
            else
    ...
    flashlightSection:AddToggle({
        ...
        Callback = function(v)
            VD.FLASH_Laser = v
            if not v and getgenv().KUBZZ_ClearFlashlightLaser then
                getgenv().KUBZZ_ClearFlashlightLaser()
            end
        end
    })
...
do -- FOV Tab
    ...
    KUBZZ_AddMainInfoLine(infoPanelSection, "KubzztKiller", "Kubzzt Killer Display", "Off")
    KUBZZ_AddMainInfoLine(infoPanelSection, "KillerPerks", "Spectate Killer Perks", "Off")
    KUBZZ_AddMainInfoLine(infoPanelSection, "PredictMap", "Predict Map", "Off")

    visualSection:AddToggle({ Default = false, Name = "Kubzzt Killer Display", Flag = "Kubzzt Killer Display", Callback = function(v) 
        VD.VIS_KubzztKiller = v 
        if v then
            StartKubzztKiller()
        else
            StopKubzztKiller()
    ...
    visualSection:AddToggle({ Default = false, Name = "Hide Survivor Icon", Flag = "Hide Survivor Icon", Callback = function(v)
        if getgenv().KUBZZ_SetHideSurvivorIcon then
            getgenv().KUBZZ_SetHideSurvivorIcon(v)
        else
            VD.VIS_HideSurvivorIcon = v
        end
    end })
    visualSection:AddToggle({ Default = false, Name = "Show Ping & FPS", Flag = "Show Ping & FPS", Callback = function(v)
        if getgenv().KUBZZ_SetShowPingFPS then
            getgenv().KUBZZ_SetShowPingFPS(v)
        else
    ...
    visualSection:AddToggle({ Default = false, Name = "Show Hook Counter", Flag = "Show Hook Counter", Callback = function(v)
        if getgenv().KUBZZ_SetShowHookCounter then
            getgenv().KUBZZ_SetShowHookCounter(v)
        else
...
do -- Survivor Tab
    ...
    combatSurv:AddToggle({ Default = false, Name = "Swift Vault V2", Locked = false, Flag = "SURV_SwiftVaultV2", Callback = function(v) 
        VD.SURV_FastVault = v 
        if not v then
            local char = LocalPlayer.Character
    ...
    combatSurv:AddToggle({ Default = false, Name = "Anti Knock", Locked = false, Flag = "Anti Knock", Callback = function(v) 
        VD.SURV_AntiKnock = v 
    end })
    ...
    combatSurv:AddToggle({ Default = false, Name = "Auto Dodge Spear (Veil)", Locked = false, Flag = "Auto Dodge Spear", Callback = function(v)
        VD.SURV_AutoDodgeSpear = v
    end })
    combatSurv:AddToggle({ Default = false, Name = "Aura Heal All", Locked = false, Flag = "Auto Heal All", Callback = function(v)
        setAutoHealAll(v)
    end })
    ...
    combatSurv:AddToggle({ Default = false, Name = "Auto Parry", Locked = false, Flag = "Auto Parry", Callback = function(v)
        VD_SetAutoParry(v)
    end })
    ...
    combatSurv:AddToggle({ Default = false, Name = "Undraggable Button (Fake Parry)", Locked = false, Flag = "Undraggable Button (Fake Parry)", Callback = function(v)
    ...
    combatSurv:AddToggle({ Default = false, Name = "Undraggable Button (Fake Gen)", Locked = false, Flag = "Undraggable Button (Fake Gen)", Callback = function(v)
...
getgenv().KUBZZ_FakeParryTrack = nil
...
function VD_PlayFakeParry()
    ...
    pcall(function()
        ...
        if getgenv().KUBZZ_FakeParryTrack then
            pcall(function() getgenv().KUBZZ_FakeParryTrack:Stop() end)
            getgenv().KUBZZ_FakeParryTrack = nil
        end
        ...
        track:Play()
        getgenv().KUBZZ_FakeParryTrack = track
    end)
end

if not getgenv().KUBZZ_FakeParryInputConn then
    getgenv().KUBZZ_FakeParryInputConn = game:GetService("UserInputService").InputBegan:Connect(function(input, gp)
        if gp then return end
...
-- ==========================================
getgenv().KUBZZ_FakeGenTrack = nil
FakeGenData = {
    UI = nil,
    Button = nil,
    DragLocked = false,
...
function VD_ToggleFakeGen()
    if not VD.SURV_FakeGen then
        if getgenv().KUBZZ_FakeGenTrack then
            pcall(function() getgenv().KUBZZ_FakeGenTrack:Stop() end)
            getgenv().KUBZZ_FakeGenTrack = nil
        end
    ...
    if getgenv().KUBZZ_FakeGenTrack then
        ...
        pcall(function()
            ...
            track.Priority = Enum.AnimationPriority.Action
            track:Play()
            getgenv().KUBZZ_FakeGenTrack = track
...
do -- Killer Tab
    ...
    abilityKiller:AddToggle({ Default = false, Name = "Infinite Abyssal Burst (Abyss)", Locked = false, Flag = "Infinite Abyssal Burst (Abyss)", Callback = function(v)
        VD.KILLER_BypassCooldown = v
        if v then
            KUBZZ_StartAbyssCooldownBypass()
        else
            KUBZZ_StopAbyssCooldownBypass()
        end
    end })
    abilityKiller:AddToggle({ Default = false, Name = "Infinite Skill (Hidden)", Locked = false, Flag = "Infinite Skill (Hidden)", Callback = function(v)
        VD.KILLER_BypassLeap = v
        if v then
            pcall(KUBZZ_StartHiddenCooldownBypass)
        else
            pcall(KUBZZ_StopHiddenCooldownBypass)
        end
    end })
    abilityKiller:AddToggle({ Default = false, Name = "Infinite Frenzy (Jeff)", Locked = false, Flag = "Infinite Frenzy (Jeff)", Callback = function(v)
        VD.KILLER_InfFrenzy = v
        if v then
            pcall(KUBZZ_StartJeffCooldownBypass)
        else
            pcall(KUBZZ_StopJeffCooldownBypass)
        end
    end })

    abilityKiller:AddToggle({ Default = false, Name = "Infinite Lake Mist (Jason)", Locked = false, Flag = "Infinite Lake Mist (Jason)", Callback = function(v)
        VD.KILLER_InfLakeMist = v
        if v then
            pcall(KUBZZ_StartSlasherCooldownBypass)
        else
            pcall(KUBZZ_StopSlasherCooldownBypass)
        end
    end })

    abilityKiller:AddToggle({ Default = false, Name = "Infinite Pursuit (Jason)", Locked = false, Flag = "Infinite Pursuit (Jason)", Callback = function(v)
    ...
    abilityKiller:AddToggle({ Default = false, Name = "Infinite Grab (Myers)", Locked = false, Flag = "Infinite Grab (Myers)", Callback = function(v)
        setMyersGrab(v)
    end })
    
    abilityKiller:AddToggle({ Default = false, Name = "Fake Attack (Counter Parry)", Locked = false, Flag = "Fake Attack (Counter Parry)", Callback = function(v)
        VD.KILLER_FakeAttack = v
        pcall(KUBZZ_ToggleFakeAttack, v)
    end })

    abilityKiller:AddToggle({ Default = false, Name = "Undraggable Button (Inf Grab)", Locked = false, Flag = "Undraggable Button (Inf Grab)", Callback = function(v)
    ...
    pcall(function()
        ...
        abilityKiller:AddButton({
            ...
            Locked = false,
            Callback = function()
                pcall(KUBZZ_ApplyCustomMasked, VD.KILLER_CustomMasked)
            end
        })
        abilityKiller:AddButton({
            ...
            Callback = function()
                ...
                pcall(KUBZZ_ApplyCustomMasked, mask)
            end
        })
    end)
    ...
    utilKiller:AddToggle({ Default = false, Name = "Block All Vaults", Locked = false, Flag = "Block All Vaults", Callback = function(v)
        VD.KILLER_BlockVaults = v
    end })
    utilKiller:AddToggle({ Default = false, Name = "Auto Drop All Pallets", Locked = false, Flag = "Auto Drop All Pallets", Callback = function(v)
        VD.KILLER_BlockPallets = v
    end })
    utilKiller:AddToggle({ Default = false, Name = "Break All Pallet", Locked = false, Flag = "Break All Pallet", Callback = function(v)
...
do -- Escape Tab
    ...
    escapeSurv:AddToggle({ Default = false, Name = "Beat Survivor (auto exit)", Locked = false, Flag = "Beat Survivor (auto exit)", Callback = function(v) 
        VD.BEAT_Survivor = v 
    end })
...
do -- Generator Tab
    ...
    genAuto:AddToggle({ Default = false, Name = "Boost Gen Bypass", Locked = false, Flag = "Boost Gen Bypass", Callback = function(v)
        setGenBypass(v)
    end })
    genAuto:AddDropdown({
...
do -- Fling Tab
    ...
    flingSection:AddToggle({ Default = false, Name = "Enable Fling", Locked = false, Flag = "Enable Fling", Callback = function(v) 
        VD.FLING_Enabled = v 
    end })
    ...
    flingSection:AddButton({ Name = "Fling Nearest", Locked = false, Callback = function() 
        pcall(function() KUBZZ_FlingNearest() end) 
    end })
    flingSection:AddButton({ Name = "Fling All", Locked = false, Callback = function() 
        pcall(KUBZZ_FlingAll) 
    end })
end
...
print("KUBZZ HUB Violence District v1.5.7 Loaded")
pcall(function()
    VD_Notify("KubzzHub", "Violence District v1.5.7 Loaded Successfully!", 5)
end)
...
function KUBZZ_ApplyCustomMasked(maskName)
...
function VD_ClearSurvivorWarnings()
    for _, player in ipairs(Players:GetPlayers()) do
        ...
        local root = char and char:FindFirstChild("HumanoidRootPart")
        local warn = root and root:FindFirstChild("KUBZZ_SurvivorKillerWarn")
...
function VD_UpdateSurvivorWarnings()
    ...
    local now = tick()
    if VD._WarnKillerKubzzt and now < VD._WarnKillerKubzzt then return end
    VD._WarnKillerKubzzt = now + 0.15
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if IsSurvivor(player) and player.Character then
            ...
            if root and hum and hum.Health > 0 then
                ...
                local warn = root:FindFirstChild("KUBZZ_SurvivorKillerWarn")
                if nearest <= 60 then
                    local danger = nearest <= 40
                    if not warn then
                        warn = Instance.new("BillboardGui")
                        warn.Name = "KUBZZ_SurvivorKillerWarn"
                        warn.Adornee = root
...
function VD_UpdateBypassGate()
    ...
    if VD._KubzztBypassGate and tick() < VD._KubzztBypassGate then return end
    VD._KubzztBypassGate = tick() + 1
...
function VD_CreateMoonwalkButton()
    ...
    local old = parent:FindFirstChild("KUBZZ_MoonwalkButton")
    if old then pcall(function() old:Destroy() end) end

    local sg = Instance.new("ScreenGui")
    sg.Name = "KUBZZ_MoonwalkButton"
    sg.ResetOnSpawn = false
    sg.IgnoreGuiInset = true
...
local function VD_CreateAimLockButton()
    ...
    local old = parent:FindFirstChild("KUBZZ_AimLockButton")
    if old then pcall(function() old:Destroy() end) end

    local sg = Instance.new("ScreenGui")
    sg.Name = "KUBZZ_AimLockButton"
    sg.ResetOnSpawn = false
    sg.IgnoreGuiInset = true
    sg.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
...
function KUBZZ_ScanMap()
    ...
    KUBZZ_Cache.Generators = newGens
    KUBZZ_Cache.Gates      = newGates
    KUBZZ_Cache.Hooks      = newHooks
    KUBZZ_Cache.Pallets    = newPallets
    KUBZZ_Cache.Windows    = newWindows
    KUBZZ_Cache.ExitPos    = exitPos
    KUBZZ_Cache.ExitPart   = exitPart
    print("[KUBZZ ScanMap] Generators:", #newGens, "Gates:", #newGates, "Hooks:", #newHooks, "Windows:", #newWindows)

    local root           = Root
    if root and #KUBZZ_Cache.Hooks > 0 then
        local closest, closestDist = nil, math.huge
        for _, hook in ipairs(KUBZZ_Cache.Hooks) do
            if hook.part then
        ...
        KUBZZ_Cache.ClosestHook = closest
    end
end
...
function CreateRadarGUI()
    ...
    radarGui.Name = "KubzzHub_RadarGUI"
...
function UpdateRadar()
    ...
    if VD.RADAR_ShowGenerator then
        for _, gen in ipairs(KUBZZ_Cache.Generators or {}) do
    ...
    if VD.RADAR_ShowPallet then
        for _, pallet in ipairs(KUBZZ_Cache.Pallets or {}) do
    ...
    if VD.RADAR_ShowHook then
        for _, hook in ipairs(KUBZZ_Cache.Hooks or {}) do
    ...
    if VD.RADAR_ShowGate then
        for _, gate in ipairs(KUBZZ_Cache.Gates or {}) do
    ...
    if VD.RADAR_ShowWindow then
        for _, window in ipairs(KUBZZ_Cache.Windows or {}) do
    ...
    if VD.RADAR_ShowZombie then
        if KUBZZ_WorldReg and KUBZZ_WorldReg.SCPZombie then
            for model, entry in pairs(KUBZZ_WorldReg.SCPZombie) do
...
function KUBZZ_TeleportToPosition(pos)
...
function KUBZZ_TeleportToGenerator(index)
    if not KUBZZ_Cache or not KUBZZ_Cache.Generators or #KUBZZ_Cache.Generators == 0 then print("[KUBZZ HUB] Generator tidak ditemukan") return false end

    local sorted = {}
    for _, gen in ipairs(KUBZZ_Cache.Generators) do
    ...
    return KUBZZ_TeleportToPosition(target.gen.part.Position)
end

function KUBZZ_TeleportToGate()
    if not KUBZZ_Cache or not KUBZZ_Cache.Gates or #KUBZZ_Cache.Gates == 0 then print("[KUBZZ HUB] Gate tidak ditemukan") return false end
    local closest, closestDist = nil, math.huge
    for _, gate in ipairs(KUBZZ_Cache.Gates) do
    ...
    return KUBZZ_TeleportToPosition(closest.part.Position)
end

function KUBZZ_TeleportToHook()
    if not KUBZZ_Cache or not KUBZZ_Cache.ClosestHook then print("[KUBZZ HUB] Hook tidak ditemukan") return false end
    return KUBZZ_TeleportToPosition(KUBZZ_Cache.ClosestHook.part.Position)
end
...
function CheckMapChange()
    ...
    if CurrentMapName ~= mapName then
        -- print("[KUBZZ HUB] Map berubah: " .. tostring(CurrentMapName) .. " -> " .. mapName)
        ...
        if descendant:IsA("Model") or descendant:IsA("Folder") then
            ...
            if n:find("generator") or n:find("mesin") or n:find("pallet") or n:find("window") or n:find("hook") or n:find("gate") then
                task.delay(0.5, function()
                    ...
                    if part then
                        if n:find("generator") or n:find("mesin") then table.insert(KUBZZ_Cache.Generators, {model=descendant, part=part})
                        elseif n:find("pallet") then table.insert(KUBZZ_Cache.Pallets, {model=descendant, part=part})
                        elseif n:find("window") then table.insert(KUBZZ_Cache.Windows, {model=descendant, part=part})
                        elseif n:find("hook") then table.insert(KUBZZ_Cache.Hooks, {model=descendant, part=part})
                        elseif n:find("gate") then table.insert(KUBZZ_Cache.Gates, {model=descendant, part=part})
                        end
                    end
                end)
            end
...
Workspace.ChildAdded:Connect(function(child)
    ...
    if child and child.Name == "Spearprojectile" and VD.SURV_AutoDodgeSpear and GetRole() == "Survivor" then
        task.spawn(function()
            if getgenv().KUBZZ_IsDodging then return end
...
local function KUBZZ_AutoAttack()
...
RunService.Heartbeat:Connect(function()
    ...
    pcall(function()
        ...
        local windowGroups = {}
        for _, win in ipairs(KUBZZ_Cache.Windows or {}) do
...
RunService.Heartbeat:Connect(function()
    ...
    pcall(function()
        ...
        -- Scan pallet dari KUBZZ_Cache + langsung dari workspace
        ...
        -- Iterasi KUBZZ_Cache.Pallets untuk cari Palletwrong terdekat
        for _, pal in ipairs(KUBZZ_Cache.Pallets or {}) do
...
local function KUBZZ_UpdateHitboxes()
...
local function KUBZZ_DestroyAllPallets()
...
-- =====================================================
getgenv().KUBZZ_IsBreakingGenerator = false

function KUBZZ_AutoBreakGene()
    if not VD.KILLER_AutoBreakGene or GetRole() ~= "Killer" then return end
    if getgenv().KUBZZ_IsBreakingGenerator then return end
    ...
    if nearest then
        getgenv().KUBZZ_IsBreakingGenerator = true
        task.spawn(function()
            pcall(function()
                local r = ReplicatedStorage:FindFirstChild("Remotes")
...
-- =====================================================
getgenv().KUBZZ_LastVaultBlockTime = 0

function KUBZZ_BlockAllVaults()
    if not VD.KILLER_BlockVaults or GetRole() ~= "Killer" then return end
    
    local now = tick()
    if now - getgenv().KUBZZ_LastVaultBlockTime < 1.5 then return end
    getgenv().KUBZZ_LastVaultBlockTime = now
    ...
    pcall(function()
        ...
        if vaultsFolder then
            ...
            -- Fallback using KUBZZ_Cache.Windows if Map structure is not default
            for _, win in ipairs(KUBZZ_Cache.Windows or {}) do
...
-- =====================================================
getgenv().KUBZZ_LastPalletBlockTime = 0

function KUBZZ_BlockAllPalletDrops()
    if not VD.KILLER_BlockPallets or GetRole() ~= "Killer" then return end

    local now = tick()
    if now - getgenv().KUBZZ_LastPalletBlockTime < 2 then return end
    getgenv().KUBZZ_LastPalletBlockTime = now
    ...
    pcall(function()
        ...
        -- Fallback: gunakan KUBZZ_Cache.Pallets
        for _, pal in ipairs(KUBZZ_Cache.Pallets or {}) do
...
-- =====================================================
getgenv().KUBZZ_LastPalletBlockDropTime = 0
getgenv().KUBZZ_IsBlockingPallets = false

-- Helper: force bersihkan semua attribute yang bisa bikin stuck
local function KUBZZ_ForceUnstuck(char)
    pcall(function()
...
function KUBZZ_BlockPalletDrop()
    if not VD.KILLER_BlockPalletDrop or GetRole() ~= "Killer" then return end
    if getgenv().KUBZZ_IsBlockingPallets then return end

    local now = tick()
    if now - getgenv().KUBZZ_LastPalletBlockDropTime < 4 then return end
    getgenv().KUBZZ_LastPalletBlockDropTime = now
    ...
    pcall(function()
        ...
        getgenv().KUBZZ_IsBlockingPallets = true
...
-- =====================================================
getgenv().KUBZZ_AbyssCooldownBypassConnection = nil
getgenv().KUBZZ_CorruptHandlerFunc = nil

function KUBZZ_StartAbyssCooldownBypass()
    if not getgenv().KUBZZ_CorruptHandlerFunc then
        for _, v in pairs(getgc(true)) do
            if type(v) == "function" and islclosure(v) then
                local constants = debug.getconstants(v)
                if table.find(constants, "corrupt") and table.find(constants, "Immobile") then
                    getgenv().KUBZZ_CorruptHandlerFunc = v
                    break
                end
            end
        end
    ...
    if getgenv().KUBZZ_AbyssCooldownBypassConnection then 
        getgenv().KUBZZ_AbyssCooldownBypassConnection:Disconnect() 
    end
    
    getgenv().KUBZZ_AbyssCooldownBypassConnection = RunService.Heartbeat:Connect(function()
        if not VD.KILLER_BypassCooldown then return end
        if getgenv().KUBZZ_CorruptHandlerFunc then
            local upvalues = debug.getupvalues(getgenv().KUBZZ_CorruptHandlerFunc)
            for idx, val in pairs(upvalues) do
                if type(val) == "boolean" then
                    if val == false then
                        debug.setupvalue(getgenv().KUBZZ_CorruptHandlerFunc, idx, true)
                    end
                end
            end
        end
...
function KUBZZ_StopAbyssCooldownBypass()
    if getgenv().KUBZZ_AbyssCooldownBypassConnection then
        getgenv().KUBZZ_AbyssCooldownBypassConnection:Disconnect()
        getgenv().KUBZZ_AbyssCooldownBypassConnection = nil
    end
...
-- =====================================================
getgenv().KUBZZ_JeffCooldownBypassThread = nil

function KUBZZ_StartJeffCooldownBypass()
    if getgenv().KUBZZ_JeffCooldownBypassThread then return end
    getgenv().KUBZZ_JeffCooldownBypassThread = task.spawn(function()
...
function KUBZZ_StopJeffCooldownBypass()
    pcall(function()
        local player = game:GetService("Players").LocalPlayer
        local char = player.Character
        if char and char:GetAttribute("Frenzy") == true then
...
getgenv().KUBZZ_SlasherCooldownBypassThread = nil

function KUBZZ_StartSlasherCooldownBypass()
    if getgenv().KUBZZ_SlasherCooldownBypassThread then return end
    ...
    getgenv().KUBZZ_SlasherCooldownBypassThread = task.spawn(function()
...
function KUBZZ_StopSlasherCooldownBypass()
...
-- ==============================================
getgenv().KUBZZ_FakeAttackThread = nil
function KUBZZ_ToggleFakeAttack(enabled)
    if not enabled then
        if getgenv().KUBZZ_FakeAttackThread then
            task.cancel(getgenv().KUBZZ_FakeAttackThread)
            getgenv().KUBZZ_FakeAttackThread = nil
        end
        return
    end
    
    if getgenv().KUBZZ_FakeAttackThread then return end
    
    getgenv().KUBZZ_FakeAttackThread = task.spawn(function()
        local Players = game:GetService("Players")
        local LocalPlayer = Players.LocalPlayer
...
function KUBZZ_UpdateCureFlaskLaser()
    ...
    if originPos and targetPos and actionActive then
        if not getgenv().KUBZZ_CureFlaskLaserPart then
            local laser = Instance.new("Part")
            ...
            laser.Transparency = 0
            laser.Parent = workspace
            getgenv().KUBZZ_CureFlaskLaserPart = laser
        end
        
        local dist = (targetPos - originPos).Magnitude
        if dist > 0.1 then
            local laser = getgenv().KUBZZ_CureFlaskLaserPart
            laser.Size = Vector3.new(0.16, 0.16, dist)
            laser.CFrame = CFrame.new((originPos + targetPos) / 2, targetPos)
            laser.Transparency = 0
        end
        ...
        if getgenv().KUBZZ_CureFlaskLaserPart then
            getgenv().KUBZZ_CureFlaskLaserPart.Transparency = 1
        end
    end
end

function KUBZZ_StartCureFlaskLaser()
    if getgenv().KUBZZ_CureFlaskLaserThread then return end
    getgenv().KUBZZ_CureFlaskLaserThread = game:GetService("RunService").RenderStepped:Connect(function()
        ...
        pcall(KUBZZ_UpdateCureFlaskLaser)
    end)
end
...
-- =====================================================
getgenv().KUBZZ_HiddenLeapBypassThread = nil

function KUBZZ_StartHiddenCooldownBypass()
    if getgenv().KUBZZ_HiddenLeapBypassThread then return end
    getgenv().KUBZZ_HiddenLeapBypassThread = task.spawn(function()
...
function KUBZZ_StopHiddenCooldownBypass()
...
-- =====================================================
local function KUBZZ_BeatGameSurvivor()
    ...
    if not exitPos and map then
        pcall(function()
            ...
            if KUBZZ_Cache and KUBZZ_Cache.ExitPos then
                exitPos = KUBZZ_Cache.ExitPos
                finishPart = KUBZZ_Cache.ExitPart
                return
            end
        end)
    end
...
-- =====================================================
local function KUBZZ_BeatGameKiller()
...
local function KUBZZ_AutoHook()
    ...
    if isCarrying then
        ...
        for _, h in ipairs(KUBZZ_Cache.Hooks or {}) do
            if h.part then
        ...
        if player ~= LocalPlayer and IsSurvivor(player) and player.Character then
            ...
            if tr and hum then
                ...
                if pct <= 0.25 and pct > 0 then
                    ...
                    if KUBZZ_Cache and KUBZZ_Cache.Hooks then
                        for _, hh in ipairs(KUBZZ_Cache.Hooks) do
...
task.spawn(function()
    while not VD.Destroyed do
        if Root and KUBZZ_Cache.Hooks and #KUBZZ_Cache.Hooks > 0 then
            local closest, closestDist = nil, math.huge
            for _, hook in ipairs(KUBZZ_Cache.Hooks) do
                if hook.part then
                    local d = (hook.part.Position - Root.Position).Magnitude
...
task.spawn(function()
    while not VD.Destroyed do
        pcall(KUBZZ_AutoAttack)
        pcall(KUBZZ_UpdateHitboxes)
        pcall(KUBZZ_DestroyAllPallets)
        pcall(KUBZZ_AutoBreakGene)
        pcall(KUBZZ_BlockAllVaults)
        pcall(KUBZZ_BlockAllPalletDrops)
        pcall(KUBZZ_BlockPalletDrop)

        pcall(KUBZZ_BeatGameSurvivor)
        pcall(KUBZZ_BeatGameKiller)
        pcall(KUBZZ_AutoHook)
        task.wait(0.12)
    end
end)
...
local function KUBZZ_IsTouchOnAttackButton(input)
    local pos = input.Position
    local playerGui = LocalPlayer:FindFirstChild("PlayerGui")
...
UserInputService.InputBegan:Connect(function(input, gpe)
    ...
    if VD.AIM_Enabled and VD.AIM_UseRMB then
        ...
        elseif input.UserInputType == Enum.UserInputType.Touch and not gpe then
            ...
            if KUBZZ_IsTouchOnAttackButton(input) then
                State.AimHolding = true
            end
        end
    end
...
do -- Teleport Tab
    ...
    tpMapSection:AddButton({ Name = "TP to Gen", Callback = function() pcall(function() KUBZZ_TeleportToGenerator(1) end) end })
    tpMapSection:AddButton({ Name = "TP to Gate", Callback = function() pcall(KUBZZ_TeleportToGate) end })
    tpMapSection:AddButton({ Name = "TP to Hook", Callback = function() pcall(KUBZZ_TeleportToHook) end })
end
...
-- =====================================================
-- KUBZZT KILLER DISPLAY
-- =====================================================
getgenv().KUBZZ_KubzztKillerGui = nil
getgenv().KUBZZ_KubzztKillerRunning = false

function SetupKubzztKillerIndicator()
    if getgenv().KUBZZ_KubzztKillerGui then pcall(function() getgenv().KUBZZ_KubzztKillerGui:Destroy() end) end
    getgenv().KUBZZ_KubzztKillerGui = nil
    KUBZZ_SetMainInfoPanelText("KubzztKiller", "Kubzzt Killer Display", "Waiting...")
end

function StartKubzztKiller()
    if getgenv().KUBZZ_KubzztKillerRunning then return end
    getgenv().KUBZZ_KubzztKillerRunning = true
    SetupKubzztKillerIndicator()
    task.spawn(function()
        local _genv = getgenv()
        while _genv.VD and _genv.VD.VIS_KubzztKiller and _genv.KUBZZ_KubzztKillerRunning do
            ...
            if nk then
                KUBZZ_SetMainInfoPanelText("KubzztKiller", "Kubzzt Killer Display", "Kubzzt Killer: " .. (nk == LocalPlayer and "YOU" or nk.DisplayName or nk.Name))
            else
                KUBZZ_SetMainInfoPanelText("KubzztKiller", "Kubzzt Killer Display", "Kubzzt Killer: None")
            end
            task.wait(2)
        end
    end)
...
function StopKubzztKiller()
    getgenv().KUBZZ_KubzztKillerRunning = false
    if getgenv().KUBZZ_KubzztKillerGui then
        pcall(function() getgenv().KUBZZ_KubzztKillerGui:Destroy() end)
        getgenv().KUBZZ_KubzztKillerGui = nil
    end
    KUBZZ_SetMainInfoPanelText("KubzztKiller", "Kubzzt Killer Display", "Off")
end

-- =====================================================
-- SPECTATOR COUNTER
-- =====================================================
if getgenv().KUBZZ_SpectatorCounterGui then
    pcall(function() getgenv().KUBZZ_SpectatorCounterGui:Destroy() end)
end
getgenv().KUBZZ_SpectatorCounterGui = nil
getgenv().KUBZZ_SpectatorCounterRunning = false

function SetupSpectatorCounter()
    if getgenv().KUBZZ_SpectatorCounterGui then pcall(function() getgenv().KUBZZ_SpectatorCounterGui:Destroy() end) end

    local sg = Instance.new("ScreenGui")
    sg.Name = "KUBZZ_SpectatorCounterGui"
    sg.ResetOnSpawn = false
    ...
    local pg = GetSafeGuiParent()
    if pg then
        local oldGui = pg:FindFirstChild("KUBZZ_SpectatorCounterGui")
    ...
    getgenv().KUBZZ_SpectatorCounterGui = sg
end

function StartSpectatorCounter()
    if getgenv().KUBZZ_SpectatorCounterRunning then return end
    getgenv().KUBZZ_SpectatorCounterRunning = true
    SetupSpectatorCounter()

    task.spawn(function()
        local _genv = getgenv()
        while _genv.VD and _genv.VD.VIS_SpectatorCounter and _genv.KUBZZ_SpectatorCounterRunning and getgenv().KUBZZ_SpectatorCounterGui do
            local sg = getgenv().KUBZZ_SpectatorCounterGui
...
if getgenv().KUBZZ_KillerPerksGui then
    pcall(function() getgenv().KUBZZ_KillerPerksGui:Destroy() end)
end
getgenv().KUBZZ_KillerPerksGui = nil

local KUBZZ_KillerPerksCollapsed = false
local KUBZZ_KillerPerksLocked = false
getgenv().KUBZZ_KillerPerksRunning = false

local KUBZZ_KillerPerkNames = {
    KubzztInLine = "Kubzzt in Line",
    ["Kubzzt in Line"] = "Kubzzt in Line",
...
function KUBZZ_EscapeRichText(text)
    text = tostring(text or "")
    text = text:gsub("&", "&amp;")
    text = text:gsub("<", "&lt;")
    text = text:gsub(">", "&gt;")
...
local function KUBZZ_FormatPerkName(name)
    name = tostring(name or "")
    if KUBZZ_KillerPerkNames[name] then return KUBZZ_KillerPerkNames[name] end
...
local function KUBZZ_GetKillerPlayer()
    for _, player in ipairs(Players:GetPlayers()) do
...
local function KUBZZ_AddWorkspacePerk(result, seen, rawName, displayName, level)
    ...
    table.insert(result, {
        Raw = rawName,
        Name = displayName and tostring(displayName) or KUBZZ_FormatPerkName(rawName),
        Level = level and tostring(level) or nil,
    })
end

local function KUBZZ_IsPerkContainer(inst)
...
local function KUBZZ_ParseWorkspacePerkName(name)
    name = tostring(name or "")
...
local function KUBZZ_ReadPerksFromWorkspace(killer)
        ...
        for key, value in pairs(attrs) do
            ...
            if lowerKey:find("perk") then
                if type(value) == "string" then
                    KUBZZ_AddWorkspacePerk(result, seen, value)
                elseif value == true then
                    KUBZZ_AddWorkspacePerk(result, seen, key)
                elseif type(value) == "number" and lowerKey:find("level") then
                    local baseName = tostring(key):gsub("[Ll]evel", ""):gsub("[Pp]erk", "")
                    if baseName ~= "" then
                        KUBZZ_AddWorkspacePerk(result, seen, baseName, nil, value)
                    end
                end
            end
        end
    ...
    for _, child in ipairs(char:GetChildren()) do
        local perkName, level = KUBZZ_ParseWorkspacePerkName(child.Name)
        if perkName then
            KUBZZ_AddWorkspacePerk(result, seen, child.Name, perkName, level)
        end
    end
    ...
    for _, inst in ipairs(char:GetDescendants()) do
        ...
        if KUBZZ_IsPerkContainer(inst) then
            for _, child in ipairs(inst:GetChildren()) do
                local value, level = readValueObject(child)
                KUBZZ_AddWorkspacePerk(result, seen, value or child.Name, nil, level)
            end
        else
            local lowerName = inst.Name:lower()
            if lowerName:find("perk") then
                local value, level = readValueObject(inst)
                KUBZZ_AddWorkspacePerk(result, seen, value or inst.Name, nil, level)
            end
        end
    end
...
local function KUBZZ_BuildKillerPerksText()
    local killer = KUBZZ_GetKillerPlayer()
    local killerName = killer and (killer.DisplayName or killer.Name) or "Unknown"
    local perks = KUBZZ_ReadPerksFromWorkspace(killer)

    if #perks == 0 then
        for _, player in ipairs(Players:GetPlayers()) do
            local candidatePerks = KUBZZ_ReadPerksFromWorkspace(player)
    ...
    local lines = {
        'Killer Perks [<font color="rgb(255,80,80)">' .. KUBZZ_EscapeRichText(killerName) .. '</font>]',
    }
    ...
    if #perks == 0 then
        ...
        for i = 1, math.min(#perks, 4) do
            ...
            table.insert(lines, '<font color="rgb(255,204,80)">- ' .. KUBZZ_EscapeRichText(perk.Name) .. KUBZZ_EscapeRichText(levelText) .. '</font>')
        end
    end
...
local function SetupKillerPerksDisplay()
    if getgenv().KUBZZ_KillerPerksGui then pcall(function() getgenv().KUBZZ_KillerPerksGui:Destroy() end) end
    getgenv().KUBZZ_KillerPerksGui = nil
    KUBZZ_SetMainInfoPanelText("KillerPerks", "Spectate Killer Perks", "Waiting for perk data...")
end

function StartKillerPerksDisplay()
    if getgenv().KUBZZ_KillerPerksRunning then return end
    getgenv().KUBZZ_KillerPerksRunning = true
    SetupKillerPerksDisplay()

    task.spawn(function()
        local _genv = getgenv()
        while _genv.VD and _genv.VD.VIS_KillerPerks and _genv.KUBZZ_KillerPerksRunning do
            local text = KUBZZ_BuildKillerPerksText()
            KUBZZ_SetMainInfoPanelText("KillerPerks", "Spectate Killer Perks", text)
            task.wait(1)
        end
    end)
end
...
function StopKillerPerksDisplay()
    ...
    KUBZZ_SetMainInfoPanelText("KillerPerks", "Spectate Killer Perks", "Off")
end
...
if getgenv().KUBZZ_PredictMapGui then
    pcall(function() getgenv().KUBZZ_PredictMapGui:Destroy() end)
end
if getgenv().KUBZZ_PredictMapConnections then
    for _, conn in ipairs(getgenv().KUBZZ_PredictMapConnections) do
        pcall(function() conn:Disconnect() end)
    end
end
getgenv().KUBZZ_PredictMapGui = nil
getgenv().KUBZZ_PredictMapConnections = {}
getgenv().KUBZZ_PredictMapRunning = false

local KUBZZ_PredictMapState = {
    Name = "Unknown",
...
local function KUBZZ_MapStringFromValue(value)
    if type(value) == "string" then
        return value
...
local function KUBZZ_MapDescFromValue(value)
    if type(value) ~= "table" then return nil end
    return value.Desc or value.desc or value.Description or value.description
end

local function KUBZZ_SetPredictedMap(name, desc, source)
    name = KUBZZ_MapStringFromValue(name) or name
    if not name or tostring(name) == "" then return end
    KUBZZ_PredictMapState.Name = tostring(name)
    if desc and tostring(desc) ~= "" then
        KUBZZ_PredictMapState.Desc = tostring(desc)
    end
    KUBZZ_PredictMapState.Source = source or "Detected"
end

local function KUBZZ_ReadCurrentWorkspaceMap()
    local map = Workspace:FindFirstChild("Map")
...
local function KUBZZ_TryPredictMapRemote()
    local remotes = ReplicatedStorage:FindFirstChild("Remotes")
    ...
    for _, args in ipairs(attempts) do
        ...
        if ok and data ~= nil then
            local name = KUBZZ_MapStringFromValue(data)
            local desc = KUBZZ_MapDescFromValue(data)
            if name then
                KUBZZ_SetPredictedMap(name, desc, "Predicted")
                return true
            end
        end
    end
...
local function KUBZZ_PredictMapText()
    local name = KUBZZ_EscapeRichText(KUBZZ_PredictMapState.Name or "Unknown")
    local source = KUBZZ_EscapeRichText(KUBZZ_PredictMapState.Source or "Idle")
    local phase = tostring(KUBZZ_PredictMapState.Phase or "")
    local timeLeft = KUBZZ_PredictMapState.TimeLeft
    local timerText = ""
    if timeLeft then
...
local function SetupPredictMapGui()
    if getgenv().KUBZZ_PredictMapGui then pcall(function() getgenv().KUBZZ_PredictMapGui:Destroy() end) end
    getgenv().KUBZZ_PredictMapGui = nil
    KUBZZ_SetMainInfoPanelText("PredictMap", "Predict Map", KUBZZ_PredictMapText())
end

local function KUBZZ_BindPredictMapEvents()
    local conns = getgenv().KUBZZ_PredictMapConnections
    local remotes = ReplicatedStorage:FindFirstChild("Remotes")
    ...
    if mapInfo and mapInfo:IsA("RemoteEvent") then
        table.insert(conns, mapInfo.OnClientEvent:Connect(function(a, b, c)
            local name = KUBZZ_MapStringFromValue(a) or KUBZZ_MapStringFromValue(b) or KUBZZ_MapStringFromValue(c)
            local desc = KUBZZ_MapDescFromValue(a) or KUBZZ_MapDescFromValue(b) or KUBZZ_MapDescFromValue(c)
            if name then
                KUBZZ_SetPredictedMap(name, desc, "Predicted")
            end
        end))
    ...
    if timeEvent and timeEvent:IsA("RemoteEvent") then
        table.insert(conns, timeEvent.OnClientEvent:Connect(function(phase, timeLeft)
            KUBZZ_PredictMapState.Phase = tostring(phase or "")
            KUBZZ_PredictMapState.TimeLeft = tonumber(timeLeft)
            if tostring(phase) == "Intermission" and tonumber(timeLeft) and tonumber(timeLeft) <= 20 then
                pcall(KUBZZ_TryPredictMapRemote)
            end
        end))
    end
    ...
    table.insert(conns, Workspace.ChildAdded:Connect(function(child)
        if child and child.Name == "Map" then
            task.delay(0.25, function()
                local name, desc = KUBZZ_ReadCurrentWorkspaceMap()
                if name then KUBZZ_SetPredictedMap(name, desc, "Confirmed") end
            end)
        end
    end))
end
...
function StartPredictMap()
    if getgenv().KUBZZ_PredictMapRunning then return end
    getgenv().KUBZZ_PredictMapRunning = true
    SetupPredictMapGui()
    KUBZZ_BindPredictMapEvents()

    task.spawn(function()
        while VD and VD.VIS_PredictMap and getgenv().KUBZZ_PredictMapRunning do
            local currentName, currentDesc = KUBZZ_ReadCurrentWorkspaceMap()
            if currentName then
                KUBZZ_SetPredictedMap(currentName, currentDesc, "Confirmed")
            else
                pcall(KUBZZ_TryPredictMapRemote)
            end
...
function StopPredictMap()
    ...
    getgenv().KUBZZ_PredictMapConnections = {}
    KUBZZ_SetMainInfoPanelText("PredictMap", "Predict Map", "Off")
end
...
-- =====================================================
getgenv().KUBZZ_OriginalFOV          = nil
getgenv().KUBZZ_OriginalCameraType   = nil
getgenv().KUBZZ_OriginalCameraOffset = nil
getgenv().KUBZZ_ThirdPersonWasActive = false
getgenv().KUBZZ_FOVWasActive         = false
...
function UpdateCameraFOV()
    ...
    if VD.CAM_FOVEnabled then
        if not getgenv().KUBZZ_FOVWasActive then
            getgenv().KUBZZ_OriginalFOV = cam.FieldOfView
            getgenv().KUBZZ_FOVWasActive = true
        end
        cam.FieldOfView = VD.CAM_FOV or 90
    elseif getgenv().KUBZZ_FOVWasActive then
        if getgenv().KUBZZ_OriginalFOV then cam.FieldOfView = getgenv().KUBZZ_OriginalFOV end
        getgenv().KUBZZ_OriginalFOV = nil
        getgenv().KUBZZ_FOVWasActive = false
    end
end
...
function UpdateThirdPerson()
    ...
    if shouldBeActive then
        if not getgenv().KUBZZ_ThirdPersonWasActive then
            getgenv().KUBZZ_OriginalCameraType = cam.CameraType
            local char = LocalPlayer.Character
            local hum = char and char:FindFirstChildOfClass("Humanoid")
            getgenv().KUBZZ_OriginalCameraOffset = hum and hum.CameraOffset or Vector3.new(0, 0, 0)
        end
        cam.CameraType = Enum.CameraType.Custom
        local char     = LocalPlayer.Character
        local hum      = char and char:FindFirstChildOfClass("Humanoid")
        if hum then hum.CameraOffset = Vector3.new(2, 1, 8) end
        getgenv().KUBZZ_ThirdPersonWasActive = true
    elseif getgenv().KUBZZ_ThirdPersonWasActive then
        if getgenv().KUBZZ_OriginalCameraType then
            cam.CameraType = getgenv().KUBZZ_OriginalCameraType; getgenv().KUBZZ_OriginalCameraType = nil
        end
        local char = LocalPlayer.Character
        local hum  = char and char:FindFirstChildOfClass("Humanoid")
        if hum then hum.CameraOffset = getgenv().KUBZZ_OriginalCameraOffset or Vector3.new(0, 0, 0) end
        getgenv().KUBZZ_OriginalCameraOffset = nil
        getgenv().KUBZZ_ThirdPersonWasActive = false
    end
end

getgenv().KUBZZ_shiftLockWasActive = false
...
function UpdateShiftLock()
    ...
    if VD.CAM_ShiftLock then
        ...
        getgenv().KUBZZ_shiftLockWasActive = true
        ...
        if getgenv().KUBZZ_shiftLockWasActive then
        ...
        if map then
            for _, obj in ipairs(map:GetDescendants()) do
                if obj.Name:lower():find("fog") or obj:IsA("Atmosphere") or obj:IsA("BloomEffect") or obj:IsA("BlurEffect") or obj:IsA("ColorCorrectionEffect") then
                    if not getgenv().KUBZZ_FogCache[obj] then
                        getgenv().KUBZZ_FogCache[obj] = {
    ...
    pcall(function()
        ...
        for _, obj in ipairs(lt:GetChildren()) do
            if obj:IsA("Atmosphere") or obj.Name:lower():find("fog") then
                if not getgenv().KUBZZ_FogCache[obj] then getgenv().KUBZZ_FogCache[obj] = { enabled = true, parent = obj.Parent } end
...
function RestoreFog()
    pcall(function()
        for obj, data in pairs(getgenv().KUBZZ_FogCache) do
...
function SetupAntiStunSlowdown()
    if getgenv().KUBZZ_AntiStunHooked then return end
    getgenv().KUBZZ_AntiStunHooked = true
...
function SetupNoCutsceneHook()
    if getgenv().KUBZZ_NoCutsceneHooked then return end
    getgenv().KUBZZ_NoCutsceneHooked = true
...
function KUBZZ_SetupNoCutsceneListeners()
    task.spawn(function()
...
task.spawn(KUBZZ_SetupNoCutsceneListeners)
...
-- =====================================================
getgenv().KUBZZ_MobileGui = getgenv().KUBZZ_MobileGui or { AimBtn=nil, FOVFrame=nil, FOVStroke=nil }
...
function CreateMobileUI()
    ...
    fovStk.Parent = fovF
    getgenv().KUBZZ_MobileGui.FOVFrame = fovF; getgenv().KUBZZ_MobileGui.FOVStroke = fovStk

    -- === AIMBOT BUTTON (ScreenGui terpisah agar AlwaysOnTop) ===
    local aimSG = Instance.new("ScreenGui")
    aimSG.Name           = "KUBZZ_AimBtn"
    ...
    getgenv().KUBZZ_MobileGui.AimBtn = btn
end
...
function setupSpearAimbotBtn()
    ...
    print("[KubzzHub] Spear Aimbot Button created successfully")
end
...
function UpdateMobileFOV()
    if not getgenv().KUBZZ_MobileGui.FOVFrame then return end
    if VD.AIM_Enabled and VD.AIM_ShowFOV then
        local r = (VD.AIM_FOV or 120)
        getgenv().KUBZZ_MobileGui.FOVFrame.Size = UDim2.new(0, r*2, 0, r*2)
        getgenv().KUBZZ_MobileGui.FOVStroke.Color = State.AimTarget and Color3.fromRGB(90,220,120) or Color3.fromRGB(220,70,70)
        getgenv().KUBZZ_MobileGui.FOVFrame.Visible = true
    else
        getgenv().KUBZZ_MobileGui.FOVFrame.Visible = false
    end
end
...
RunService.Heartbeat:Connect(function(deltaTime)
    ...
    if not DrawingAvailable and (not getgenv().KUBZZ_MobileGui.FOVFrame or not getgenv().KUBZZ_MobileGui.FOVFrame.Parent) then
        pcall(CreateMobileUI)
    end
    ...
    if not DrawingAvailable then
        if getgenv().KUBZZ_MobileGui.AimBtn then getgenv().KUBZZ_MobileGui.AimBtn.Visible = VD.AIM_Enabled end
...
-- =====================================================
getgenv().KUBZZ_SyncLoadedFeatures = function()
    ...
    -- Sync Kubzzt Killer Display
    if VD.VIS_KubzztKiller then
        pcall(StartKubzztKiller)
    else
        pcall(StopKubzztKiller)
    end
    ...
    if getgenv().KUBZZ_SetHideSurvivorIcon then
        pcall(getgenv().KUBZZ_SetHideSurvivorIcon, VD.VIS_HideSurvivorIcon)
    end

    -- Sync Ping & FPS Display
    if getgenv().KUBZZ_SetShowPingFPS then
        pcall(getgenv().KUBZZ_SetShowPingFPS, VD.VIS_ShowPingFPS)
    end

    -- Sync Hook Counter
    if getgenv().KUBZZ_SetShowHookCounter then
        pcall(getgenv().KUBZZ_SetShowHookCounter, VD.VIS_ShowHookCounter)
    end

    -- Sync Twist of Fate silent aim
    if getgenv().KUBZZ_SetToFSilentAim then
        pcall(getgenv().KUBZZ_SetToFSilentAim, VD.TOF_SilentAim)
    end

    -- Sync Flashlight silent aim
    if getgenv().KUBZZ_SetFlashlightSilentAim then
        pcall(getgenv().KUBZZ_SetFlashlightSilentAim, VD.FLASH_SilentAim)
    end
...
getgenv().KUBZZ_SyncUILibraryConfigRuntime = function()
    local tofValue = readConfigElementValue("Silent Aim Twist Of Fate")
    if type(tofValue) == "boolean" and tofValue ~= VD.TOF_SilentAim and getgenv().KUBZZ_SetToFSilentAim then
        pcall(getgenv().KUBZZ_SetToFSilentAim, tofValue)
    elseif type(tofValue) == "boolean" then
        VD.TOF_SilentAim = tofValue
        if tofValue and getgenv().KUBZZ_SetToFSilentAim then
            pcall(getgenv().KUBZZ_SetToFSilentAim, true)
        end
    end

    local flashValue = readConfigElementValue("Silent Aim Flashlight")
    if type(flashValue) == "boolean" and flashValue ~= VD.FLASH_SilentAim and getgenv().KUBZZ_SetFlashlightSilentAim then
        pcall(getgenv().KUBZZ_SetFlashlightSilentAim, flashValue)
    elseif type(flashValue) == "boolean" then
        VD.FLASH_SilentAim = flashValue
        if flashValue and getgenv().KUBZZ_SetFlashlightSilentAim then
            pcall(getgenv().KUBZZ_SetFlashlightSilentAim, true)
        end
    end
end
